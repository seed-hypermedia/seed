...
- **Quality checks are mandatory.** After any change, run the relevant lint, typecheck/vet, and test commands for every workspace touched, and report the results. Run the per-workspace test script for every workspace touched — the root `pnpm test` aggregate only covers `web`, `@shm/shared`, and desktop unit, so for other pnpm packages use `pnpm --filter <name> test`. Do not declare work done if any check fails. If a check doesn't exist for a given workspace, say so instead of skipping silently.
