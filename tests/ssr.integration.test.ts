...
import {createDocumentUpdate, createRedirectDocument} from '../frontend/apps/cli/src/test/account-helpers'
import {FIXTURE_ACCOUNT} from '../frontend/apps/cli/src/test/fixture-seed'
import {flattenToOperations, parseMarkdown} from '../frontend/apps/cli/src/utils/markdown'
...
describe('SSR Integration', () => {
  ...
  it(
    ...
    async () => {
      ...
      expect(hasText, 'Expected to find text "asdfg" in server-rendered .ssr-content-placeholder HTML').toBe(true)
    },
    TEST_TIMEOUT,
  )
...
describe('SSR Redirect Routes', () => {
  ...
  async function loaderData(path: string) {
    const response = await fetch(`${env.web.baseUrl}/${path}?_data=${encodeURIComponent('routes/$')}`)
    expect(response.status).toBe(200)
  ...
  function entityQueryFor(data: any, hmId: string) {
    return data.dehydratedState?.queries.find((q: any) => q.queryKey[0] === 'ENTITY' && q.queryKey[1] === hmId)
  }
  ...
  beforeAll(async () => {
    await createDocumentUpdate(env.web.baseUrl, FIXTURE_ACCOUNT, targetPath, titled(targetTitle, targetBody))
...
describe('SSR API Integration', () => {
  it(
    ...
    async () => {
      const response = await fetch(`http://localhost:${env.daemon.config.httpPort}/debug/version`)
