...
`publish.mjs` turns this repo into live Onyx types: <!-- id:ilHK6ydh -->
  1. Parse each `.json` file (dag-json). `ref`s are already **`hm://` URLs** — names, _not_ CIDs — so recursive and mutually-recursive schemas keep working (see [references](hm://z6MkmZUb4K5c17zGGBuJJerwFzBaGkiYLfEEnkb9CH1W1ptb/references)). They are **not** rewritten. <!-- id:3NjmpKsi -->
