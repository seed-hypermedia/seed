...
- **A typed, self-documenting API.** Every read method of the Seed API is published as a schema that pins its method key and types its input and output. The in-app API console is generated from that catalog rather than hand-wired. See [the typed API](./api.md). <!-- id:PQWGKvZO -->
...
Read [how Onyx works](./how-it-works.md) for the system end to end, [typed documents](./typed-documents.md) for the document-binding model, or go straight to the reference chapters from the [home page](./onyx.md). <!-- id:mdWI56L4 -->
