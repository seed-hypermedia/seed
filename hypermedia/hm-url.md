...
As a value type it is a reference to a Hypermedia [document](./protocol/documents.md), held as an `hm://` URL string. `format: hm-url` tells an editor to render it as a searchable reference that shows the target's title as a pill, instead of the raw URL. A `target` on the field names the schema the referenced document should be typed by: the search then offers documents whose effective attributes schema is that type or any [extension](./schema/extension.md) of it, and a reference outside that set gets a warning, never a block. <!-- id:9OE_xwGa -->
