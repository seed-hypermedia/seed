...
`call`, and promoted like any builtin or lambda (see [`tools.md`](./agent-tools.md)).
...
- **Server record** (`mcp_servers`, see [`persistence.md`](./agent-persistence.md)): `config_cbor` is
...
Account-scoped, standard envelope ([`signed-api.md`](./agent-signed-api.md)):
...
The Tools tab ends with an **MCP servers** section ([`desktop-ui.md`](./agent-desktop-ui.md)): one row per account server with
...
- A server is reached with account-configured URLs and headers; the same outbound-network posture as `read https://…`
  applies — no private-network allow/deny list yet ([`security.md`](./agent-security.md)).
