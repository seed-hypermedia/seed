...
DAG-PB is compact and carries almost no metadata: a node is a byte payload plus links. This is on purpose. The blocks hold no filename, MIME type or size. A File [block](./blocks.md) in a document may carry a display name beside the link. The daemon serves the bytes as `application/octet-stream`, and the client detects the type. File blocks are not signed. Their integrity comes from the hash alone, and their authorship from the signed [blob](../blob.md) that links to them. <!-- id:-mKRWYdr -->
