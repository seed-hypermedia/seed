...
---
A picture in a document is not stored inside the document. It is split into content-addressed blocks, given a [CID](../cid.md), and the document only carries a link `ipfs://<cid>`. The blocks travel over the same [network](./network.md) as everything else, and any Seed node that holds them can serve the file back at `/ipfs/<cid>`. <!-- id:yP2vctFk -->

This is the [IPFS](https://docs.ipfs.tech/concepts/) file model, used unchanged: Seed adds no file blob type of its own. What Seed adds is the rule for who may fetch a file, which follows the documents that link to it. <!-- id:LlqesciI -->
...
<!-- id:HQWPBmY4 -->
- **Blocks**: `Image`, `Video` and `File` blocks hold their content in `link` as `ipfs://<cid>` (see [blocks](./blocks.md)). An image in a [comment](../comment.md) works the same way. <!-- id:zLr6847r -->
- **Document metadata**: `icon`, `cover`, `thumbnail` and `seedExperimentalLogo` are `ipfs://` URLs; a [profile](../profile.md) `icon` too. <!-- id:b-PgXim7 -->
- **Schemas**: `schemaDefinition` points at a schema blob by `ipfs://<cid>`. That blob is DAG-CBOR, not a file, but it is fetched through the same gateway. <!-- id:Pr8cdxN2 -->

The [ipfs-url](../ipfs-url.md) schema type is what a typed attribute uses to say "this value is a file reference". <!-- id:VVp7-THI -->

## Visibility follows the link <!-- id:pW39Clyv -->

A file block has no visibility of its own. When it is first stored it is treated as **private**; it becomes public the moment any public blob links to it, directly or through another DAG-PB node. The indexer propagates visibility along every link type into DAG-PB and raw blocks, so a public document's image and all of its chunks are public, and a private document's image stays private. If the only document linking a file is deleted or made private, the file follows. <!-- id:6Buine9d -->

The consequence trips people up: **a file uploaded on its own is not public**. Publishing the blocks stores them, and the uploader can fetch them back, but a gateway running in public-only mode answers `404` with "blob … is not public" until a published document, comment or profile references the `ipfs://` URL. This is the rule that stops a Seed node from being anonymous public file hosting. Link the file from something public, and it appears. <!-- id:Iuhj83rQ -->
...
| --- | --- |
| `GET /ipfs/<cid>` | Look locally first, then on the network over Bitswap for up to one minute (`408` on timeout). A UnixFS root is reassembled into the whole file; any other block is returned raw. <!-- id:1trBmp0w --> |
| `GET /ipfs/<cid>?filename=photo.jpg` | Adds `Content-Disposition: attachment; filename="photo.jpg"`. <!-- id:ZvQHYexw --> |
| `Range: bytes=…` | Honoured with `206 Partial Content`; `Accept-Ranges: bytes` is always sent, which is what lets video seek. <!-- id:F-NzKWDk --> |
...
Responses carry `ETag: <cid>` and `Content-Type: application/octet-stream`; the file's real type is left to the client. `Cache-Control` is `public, max-age=29030400, immutable` (about 336 days) when the daemon can prove the blob is public, and `private, max-age=29030400, immutable` otherwise, so shared caches and CDNs never keep private content. An `Authorization: Bearer` header from a signed-in web session lets the daemon serve a private file to a reader who may see its space. <!-- id:-f6HpGkq -->

Two limitations to know. Without the `-public-only` flag a daemon serves any blob it holds to anyone who knows the CID, and both `POST` routes are open to anyone who can reach the port; the Seed app's daemon binds all interfaces, so keep its HTTP port off untrusted networks. On a hosted site the proxy forwards every method on `/ipfs/*` to the daemon, so the upload routes are reachable from the internet; what gets uploaded stays private until something public links it, but it still uses disk. The compose template in this repository does not pass `-public-only` to the site daemon as of September 2026, so a hosted site's gateway currently serves a private blob to whoever presents its CID; operators who want the stricter behaviour add the flag. <!-- id:KSKS52ek -->

Content published only on Seed is not findable from public IPFS gateways such as `ipfs.io`: Seed nodes do not advertise blocks to the IPFS DHT (see [network](./network.md)), so an `https://ipfs.io/ipfs/<cid>` link works only for content someone pinned there separately. <!-- id:URxNAB6R -->
...
There are two ways to get a file into the network, and they produce the same CIDs: <!-- id:14XfNxVD -->
  ...
  2. **Chunk it yourself and publish the blocks**: the SDK's `fileToIpfsBlobs` (one file) or `filesToIpfsBlobs` (several, sharing a block store) returns the root CID and every block with its CID; hand them to `PublishBlobs` on any site. The web app, the CLI and Seed Agents all use this path, because none of them can reach a daemon's upload route on a hosted site. Each block must be sent with its SHA-256 CID, because a blob published without an explicit CID is hashed by the daemon with BLAKE2b and would not match the links pointing at it. <!-- id:nCiBW075 -->
...
Alice drops `talk.pdf` (700 KiB) into a document in the Seed app. <!-- id:F7fiJgV_ -->
  ...
  3. Alice publishes. The signed [change](../change.md) links the root CID; the indexer marks the root and, through its `dagpb/chunk` links, the three pieces as public. The publish pushes the change, the Ref and the four file blocks to Alice's site. <!-- id:EvgmAJi0 -->
  ...
  5. Bob's Seed app, subscribed to Alice's space, discovers the change on its next wave. The reconciliation lists the root and pieces among the missing CIDs; Bitswap fetches the DAG-CBOR blobs first and the PDF in the bulk media tier, so Bob's page text appears before the attachment finishes. <!-- id:JW01eMUG -->
...
[Seed Agents](../agent.md) address files with their two verbs. `read ipfs://<cid>` fetches a file into the agent's memory (or, for a DAG-CBOR object, returns the decoded value with its signature and schema check). `write ipfs://` publishes with exactly one source: `options.fromPath` for a memory file, `options.fromAttachment` for a file the user attached to the chat, or a JSON object in `content` (optionally validated against a schema and signed). It needs the agent's `publish` grant and returns the CID and `ipfs://` URL. The runtime chunks with the SDK and publishes in 3 MiB batches to the agent's configured site (hosted agents use `https://hyper.media`; the desktop's agent uses the app's daemon). The guides at `~/tools/write/ipfs` and `~/tools/write/documents` spell out the options. The same rule applies to agents as to everyone: an `ipfs://` URL on its own displays nowhere; to show a file in chat an agent references its memory path (`![caption](~/memory/path.png)`), and to publish it for the world it links the URL from a document or comment. External agents using the `seed-cli` skill publish files through `document create -f` as above. See [using Seed from your own agent](../build/agents.md). <!-- id:9ZO5ERaI -->
