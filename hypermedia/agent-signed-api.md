...
Account-scoped, like model providers. Full behavior in [`mcp.md`](./agent-mcp.md).
...
Used over `/agents/ws`. See [WebSocket subscriptions](./agent-websocket-subscriptions.md).
...
always on and can never be granted or revoked; see [the glossary](./agent-glossary.md). (The one exception is structural, not
