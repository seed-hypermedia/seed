...
When the definition's primary `signingKey` resolves to an `hm-account-key` secret, the server also creates a default enabled `user-mention` [trigger](./triggers.md) that follows that signing identity's account uid (prompt: "Respond to the mention, performing the action requested."). [Mentioning](../protocol/comments.md) the agent's account then starts a session in which it responds. This is best-effort and never blocks agent creation. Agents without a signing key get no default trigger. <!-- id:wKRI3r0c -->
