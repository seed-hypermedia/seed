...
`source_cbor` encodes `AgentTriggerSource`: document-comment, user-mention, site-update, schedule, and run-completed. Schedule triggers store interval, weekly days and time, or one-time run configuration inside this CBOR value. There is no separate schedule table. <!-- id:DySCuqjj -->
