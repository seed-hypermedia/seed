...
Configuration comes from direnv (`.env.vars`). That layer is what makes dev different from the built-in defaults: <!-- id:ppx-D0aQ -->
  ...
  - `SEED_AGENTS_DOCS_ACCOUNT_FILE="$WORKSPACE/hypermedia/.dev/account"`: the `hm-sync` pane writes its dev key's account to this file, and the server resolves `hm://hyper.media` to it. Dev agents read the knowledge base, and the default [Agent Guide](./guide.md), straight from your working copy of `hypermedia/`. Until `hm-sync` has run once, those reads fail. <!-- id:v8hCXS77 -->
...
The stack: <!-- id:S14BIIBT -->
  ...
  - **agents-stable / agents-staging / agents-dev** run with: <!-- id:Uh2MD0Rd -->
    ...
    - `SEED_AGENTS_DEFAULT_PROMPT_URL` naming the [Agent Guide](./guide.md) new agents embed. Production uses the default, the guide on hyper.media; staging names the staging copy.
    - `SEED_AGENTS_DOCS_ACCOUNT` set to the account of the published knowledge base, so `hm://hyper.media` reads resolve. The repo never names that account. It lives in the deployment config until `hm://` URLs resolve domains. <!-- id:jKOI2Ogq -->
