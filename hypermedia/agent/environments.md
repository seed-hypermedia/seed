...
The stack: <!-- id:S14BIIBT -->
  ...
  - **agents-stable / agents-staging / agents-dev** run with: <!-- id:Uh2MD0Rd -->
    ...
    - `SEED_AGENTS_DOCS_ACCOUNT` set to the account of the published knowledge base, so `hm://hyper.media` reads and the default [Agent Guide](./guide.md) resolve. The repo never names that account. It lives in the deployment config until `hm://` URLs resolve domains. <!-- id:jKOI2Ogq -->
