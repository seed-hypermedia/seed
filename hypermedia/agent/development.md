...
---
This page is for people and coding agents changing [Seed Agents](../agent.md). It names where things are, how to run and validate them, and the conventions that keep the runtime coherent. Read the root `AGENTS.md`, then `agents/AGENTS.md` for the service (Bun only, never pnpm; `bun check && bun test` before every commit) and `frontend/AGENTS.md` for UI work. General repo setup is in [contributing](../build/contributing.md). <!-- id:K7JAYkEv -->
...
`pnpm audit` fails today on repository dependency advisories unrelated to this feature. Report that. Do not claim it passed. <!-- id:ny9jUwE2 -->
...
The service, in `agents/`: <!-- id:I1toG_st -->
  ...
  - `src/runs.ts`: durable [run](./runs.md) records and the dispatch queue: leases, fair-share claiming, retries, cancellation cascade, timer wakes. <!-- id:4vcGWyxX -->
  ...
  - `src/tool-documents.ts`: tools as content-addressed [tool documents](./tool-document.md): the lambda ABI, builtin materialization, the MCP projection, authoring validation, contract markdown. <!-- id:p-4s5Rp2 -->
  - `src/mcp.ts`: remote [MCP servers](./mcp.md): connect, discover, proxy, the lazy per-run connection pool. <!-- id:xfcQtnXV -->
...
Shared Hypermedia behaviour the service reuses from [`@seed-hypermedia/client`](../build/sdk.md): `resource-read.ts` (`resolveIdWithClient`, shared with the [CLI](../build/cli.md)), `hm-resolver.ts`, `blocks-to-markdown.ts` and `markdown-to-blocks.ts`, `explore-query.ts`, and the blob signing and DAG-CBOR primitives (imported through the `@shm/shared/blobs` and `@shm/shared/cbor` re-exports). <!-- id:MF4Ylcua -->
...
The verbs (the five working verbs plus `status` and `continue_session`) are the whole provider-facing surface. New capability arrives as an address, an option, or a callable, never as another verb without a design discussion. <!-- id:c9hqLvkf -->
  ...
  2. A new **callable** goes in `callableToolRegistry` with `runtimes` including `agent-service`. It is reachable through `call` and never added to the provider payload directly. The next listing materializes it as a tool document for every agent, and the CID change is the version bump. <!-- id:3CFRldLu -->
  3. Keep touch-expand intact: a wrong or unexpanded `call` answers with the contract, and never with an error. <!-- id:FtdrC1Vt -->
  4. Anything [promoted](./promotion.md) into the provider payload must be filtered against the agent's enabled callables, because promotion is derived from durable events and an unfiltered allowlist would let a hallucinated tool name activate a real one. <!-- id:5YLbjR8m -->
  5. [Grants](./grants.md) are `publish` plus the callable set. Do not add a grant for a verb. <!-- id:5G20WmgP -->
