...
As of 2026-08-13 on `harness/full`, the "tonight" subset from [Sizing](#sizing) is built (commit `b7596f83c`, reviewed in [`reviews/05-exec-time.md`](./review-05-exec-time.md)): the `run-completed` source, the `wake` continuation, the shared activity matcher, and the firing-chain loop guard, all riding the existing `agent_triggers` rows. <!-- id:TOc56lER -->
