...
[perf-squeeze-plan.md](./agent-perf-squeeze-plan.md), which attacks host throughput; here the unit of success is the
...
1. **Keep the microVM alive between calls** — [exec-warm-pool.md](./agent-exec-warm-pool.md). Removes `exec.boot` from every
   call after the first and, far bigger, preserves guest warm state for dev loops.
2. **Cut provider round-trip cost** — [model-comms-latency.md](./agent-model-comms-latency.md). TTFT and per-turn upload: prompt
