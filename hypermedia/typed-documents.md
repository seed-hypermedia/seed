...
Every document's metadata may carry up to three schema-related fields. They are declared on the base document schema, [hypermedia-metadata](./hypermedia-metadata.md), and each one says a different thing: <!-- id:eKwMrgCU -->
...
Suppose the Acme account wants person pages. <!-- id:XIl-loiu -->
  1. Acme publishes a schema blob. It extends the base document, [hypermedia-document](./hypermedia-document.md), and requires a `surname` in `metadata`. The blob has a CID. <!-- id:tEjNJiAm -->
...
Once a document has an effective schema, the Seed app changes in four visible ways: <!-- id:SaArwfA0 -->
  ...
  - **Problems are shown in red and never block.** A per-field badge and a summary banner list the actual violations — "surname is required", "status must be one of draft, published, archived". Saving always works. Validation is a guardrail, not a gate; see [why Onyx](./why.md). <!-- id:lciGA4Bw -->
