...
The vocabulary above is defined once in [`glossary.md`](./agent-glossary.md).
...
- Local agents server lifecycle: attaches to an already-running server in development, spawns the bundled binary in a
  packaged app. See [Desktop agent unification](./agent-desktop-agent-unification.md).
...
- Triggers and plans are still SQLite rows rather than documents in the Space; the event-bus milestone that moves them
  is only partly built (see [harness/m6-event-bus-design.md](./agent-harness-m6-event-bus-design.md)).
...
See [Future projects](./agent-future-projects.md) and [Roadmap](./agent-roadmap.md).
