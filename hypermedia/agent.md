...
The runtime is built on three nouns and five verbs. The nouns: an agent's **Space** (`~/memory/`, `~/tools/`), the session **Log** (append-only, every event stamped with an actor), and the **Runs** table (every turn, child, and script, which doubles as the dispatch queue). The verbs — `read`, `write`, `call`, `delegate`, `plan` — are the entire model-facing tool surface and are always on; everything that used to be its own tool is now an address form of a verb or a callable dispatched through `call`. [`glossary.md`](./agent.md) defines this vocabulary and is the sanctioned wording for these docs; do not restate it here, point at it. <!-- id:N8Qe0rh- -->
...
1. [Glossary](./agent.md) — the sanctioned vocabulary: three nouns, five verbs, delegation, plans, tools, triggers. Read it first; the rest of these docs assume its words. <!-- id:wy6v88cs -->
2. [System overview](./agent/system-overview.md) — architecture, boundaries, lifecycle, completed/incomplete status. <!-- id:yiVgkwBy -->
3. [Implementation history](./agent/history/implementation.md) — notes on the recent commits that built this feature. <!-- id:jWmf4Ib3 -->
...
7. [Desktop agent unification](./agent/plans/desktop-agent-unification.md) — running this service locally as a desktop subprocess and replacing the old assistant runtime with agent sessions. <!-- id:ZCjihFSX -->
...
12. [Tools](./agent/tools.md) — tool-call lifecycle and `read` behavior. <!-- id:KGBqHjS9 -->
    ...
    - [Delegation budgets](./agent/plans/delegation-budgets.md) — thoroughness presets and the depth/children budget (#1070); why a hard cap is only a first step, and the proposal for budget pauses, tree budgets, and cost-denominated budgets. <!-- id:x7f0aR_N -->
...
17. [Pi SDK migration project](./agent/plans/pi-sdk-migration.md) — research and implementation plan for using Pi as the agentic loop. <!-- id:GfUgUz6t -->
18. [Agent triggers plan](./agent/plans/triggers.md) — proactive triggers that create sessions from HM activity or schedules. <!-- id:YB9LibrL -->
19. [Workflows v1 plan](./agent/plans/workflows-v1.md) — the implemented plan for durable runs, delegated children, the script engine, and the progress UX, with as-built divergences at the end. <!-- id:-7zEn_4j -->
20. [The Harness plan](./agent/history/harness/plan.md) — the milestone-by-milestone plan for the current architecture, with inline **Built:** notes wherever the build diverged, an as-built status table, and per-milestone adversarial reviews under [`harness/reviews/`](https://github.com/seed-hypermedia/seed/tree/main/hypermedia). [M6 event bus design](./agent/history/harness/m6-event-bus-design.md) covers the part that is only partly built. <!-- id:z_5l6m1I -->
21. [Future projects](./agent/plans/future-projects.md) — larger future work packages. <!-- id:hBbqVvBl -->
22. [Roadmap](./agent/plans/roadmap.md) — prioritized next steps and code-improvement opportunities. <!-- id:gYVT54tz -->
23. [Performance squeeze plan](./agent/plans/perf-squeeze.md) — the measured 2026-08 production bottlenecks (sandbox CPU, provider egress, log volume) and the workstreams removing them, including session-scoped long-lived sandboxes. <!-- id:YFOYJ6n1 -->
24. [Multi-server architecture](./agent/plans/multi-server-architecture.md) — the three-phase path off a single host, with the scaling and cost model. <!-- id:1vTtMEtQ -->
25. [Agent speed plan](./agent/plans/speed.md) — the perceived-latency project: per-stage instrumentation (`/api/perf`, `scripts/bench-exec.ts`), first measurements, and the workstreams. Its two design companions: [warm microVM pool](./agent/plans/exec-warm-pool.md) for keeping sandboxes alive between `execute` calls, and [model comms latency](./agent/plans/model-comms-latency.md) for prompt caching, server-side conversation state, and byte-stable prefixes. <!-- id:iyyVcmvY -->
26. [Run concurrency](./agent/plans/run-concurrency.md) — what limits how many runs execute at once, the 2026-09-07 measurements behind a 27-minute queue wait, and the fair-share dispatch that followed. <!-- id:KqvR5tjw -->
...
Use this routing table: <!-- id:4wzDU3Fm -->
  ...
  - new vocabulary for a mechanism → a new term page in `agent/` listed under Terms below, then use its words here <!-- id:oVJOHkJq -->
...
## The three nouns <!-- id:C84oPJN9 -->

- [Space](./agent/space.md) <!-- id:Kq4MZyrz -->
- [Log](./agent/log.md) <!-- id:g7uyfY_s -->
- [Runs](./agent/runs.md) <!-- id:sNNWBzmE -->

## The five verbs (the agent's — and your — whole tool surface) <!-- id:2IAQClOe -->

- [read](./agent/read.md) <!-- id:0Jr0g6qI -->
- [write](./agent/write.md) <!-- id:j44-PRlz -->
- [call](./agent/call.md) <!-- id:X41pWIxe -->
- [delegate](./agent/delegate.md) <!-- id:VtJvoL2I -->
- [plan](./agent/plan.md) <!-- id:u2kS_wre -->

## Delegation & orchestration <!-- id:pOSnk5Lx -->

- [child](./agent/child.md) <!-- id:9EcBLyep -->
- [brief](./agent/brief.md) <!-- id:xpHKCsu6 -->
- [typed result / return_result_](./agent/typed-result.md) <!-- id:ke5Dyb7t -->
- [script / ctx](./agent/script.md) <!-- id:z0wOtMwB -->
- [journal](./agent/journal.md) <!-- id:kub9xCVv -->
- [park / wait](./agent/park.md) <!-- id:F0fP5kBe -->
- [wake source](./agent/wake-source.md) <!-- id:_0cbctMU -->
- [continueAsNew](./agent/continue-as-new.md) <!-- id:8HgtW25E -->

## Plans & attachment <!-- id:MOdtGhIM -->

- [step](./agent/step.md) <!-- id:Frnu3RIN -->
- [attachment](./agent/attachment.md) <!-- id:TnggmFEe -->
- [batch step](./agent/batch-step.md) <!-- id:sMB3a9_C -->

## Tools <!-- id:EPvfMFEA -->

- [tool document](./agent/tool-document.md) <!-- id:4v_EgFVQ -->
- [MCP server](./agent/mcp.md) <!-- id:4hAVNENP -->
- [contract](./agent/contract.md) <!-- id:kUvFyh5w -->
- [Space index](./agent/space-index.md) <!-- id:AuCuWilK -->
- [promotion](./agent/promotion.md) <!-- id:PJb3yr65 -->
- [grants](./agent/grants.md) <!-- id:h7Q2mQxs -->

## Symmetric log <!-- id:114nfbqJ -->

- [actor](./agent/actor.md) <!-- id:VgI6ihKM -->
- [wrench palette](./agent/wrench-palette.md) <!-- id:SvCpjY0u -->

## Triggers & the bus <!-- id:TJjgtL1q -->

- [trigger](./agent/trigger.md) <!-- id:Gz4PwRlY -->
- [firing](./agent/firing.md) <!-- id:B3cXsnyX -->

## Process terms (how this gets built) <!-- id:K4Cf5_RB -->

- [checkmark](./agent/history/harness/checkmark.md) <!-- id:S6389J1H -->
- [live gate](./agent/history/harness/live-gate.md) <!-- id:mNk7JrhL -->
- [the guide](./agent/history/harness/guide.md) <!-- id:UOiuiFqX -->
