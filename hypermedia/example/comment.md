...
---
A comment with a required `text`, an author [link](../link.md) to a [person](./person.md), and `replies` that link to more comments. The type refers to itself, which works because schemas [reference each other by name](../schema/references.md).
...
- [tree](./tree.md): another self-referencing type.
- [Comment](../comment.md): the real signed comment blob.
