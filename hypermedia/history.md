...
---
This section keeps design records that are still worth reading after the decision they fed. A history page is dated and is not revised to match later code. When a record and a [concept page](./protocol.md) disagree, the concept page describes what the software does today, and the record explains how the team got there or where it wanted to go. <!-- id:SdkPNtJF -->
...
Written by Eric Vicenti in late August 2026 and added to this folder on 2026-09-02. It is a design investigation. Nothing in it was decided. The shipped model is on [Permissions](./protocol/permissions.md) and [Privacy](./protocol/privacy.md). <!-- id:rjhzp1ed -->
  ...
  - [How Privacy Works Today](./history/permissions/current-state.md) maps the [visibility](./protocol/privacy.md), [capability](./protocol/permissions.md) and authentication machinery the [daemon](./apps/daemon.md) had at the time, including its known gaps. <!-- id:iKylgTlg -->
  ...
  - [Everything Is a Grant](./history/permissions/grants.md) is the rebuilt design: one signed grant statement with explicit audiences, ordering by position in a signed chain in place of timestamps, and a plainly stated trusted-server model. <!-- id:PP2WHx3p -->
  - [V1 Proposal (Archived)](./history/permissions/v1-proposal.md) is the original proposal, kept as the subject of the critique. <!-- id:A5-CJfyG -->
