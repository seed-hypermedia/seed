...
---
**`anyOf`**: the union keyword. It holds a list of [schemas](../schema.md), and a value is valid if it matches any of them. It is one of the two composite constructs in [the schema language](./schema-language.md); [`allOf`](./allof.md) is the other, and says _and_ where this says _or_. A union whose arms are all literals is a fixed set of choices (`{anyOf: ["draft", "published"]}`), which the editors show as a dropdown. <!-- id:ppR9ujjo -->
...
- [Intersection schema](./allof.md): the other composite construct, `allOf`.
