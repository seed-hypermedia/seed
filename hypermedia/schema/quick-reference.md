...
---
Everything on this page is a link into the reference; read it top to bottom once, then use it as an index. The long versions: [why](./why.md), [how it works](./how-it-works.md), [the schema language](./schema-language.md), [typed documents](./typed-documents.md), [user stories](./user-stories.md). <!-- id:h5cwDrts -->
...
- A type has a **URL** because its home page does: `hm://acme/person` resolves through that page's `schemaDefinition` to the blob. Reference by URL to follow the type as it evolves, by CID to pin it ([pinning versus following](./typed-documents.md)). <!-- id:Yl8Ey-8l -->
