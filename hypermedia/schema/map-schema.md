...
---
A map schema types a [map](./kind.md) whose keys are arbitrary and whose values all match the schema under `values`. A map with known field names is a [struct schema](./struct-schema.md).
...
- [Struct schema](./struct-schema.md): known fields, optionally open to extra keys.
- [Closed map](./closed-map.md): a struct that rejects keys it does not name.
- [List schema](./list-schema.md): the other collection variant.
- [The data model](./data-model.md): the nine kinds, including `map`.
