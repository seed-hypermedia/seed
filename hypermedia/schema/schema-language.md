...
A `struct` names its fields in `properties`, one [property](./property.md) per field: `properties[name]` is `{value, required?, description?}` — the schema the field's value must match, whether a value must include it, and what it is for. A struct is **closed**: keys not listed are rejected. Add `values` and it is open — extra keys are allowed as long as their values match the `values` schema. A `map` has no named fields; every key's value matches `values`. So: <!-- id:8FWAU877 -->
