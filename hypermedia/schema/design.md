...
- **References are names (`hm://` URLs), not CIDs.** A CID cycle has no encoding order. Recursion, including _mutual_ recursion (`example/folder` and `example/file` point at each other), can only be expressed with names. Names resolve through a signing [authority](../authority.md). The library writes its authority as `hyper.media`, which the SDK resolves to its bundle until the network resolves domains in `hm://` URLs. CIDs pin immutable versions. This replaced an earlier self-sentinel idea. See [references](./references.md) and [URLs](../protocol/urls.md). <!-- id:l1_v0PJ1 -->
- **`type` is a URL, and it is the only naming key.** `type: "hm://hyper.media/map"` makes the kind clickable and self-explanatory. Each [primitive](./primitive.md), such as `map`, is a self-grounding axiom whose `type` names itself. The same key names another schema when the node includes or extends one, so naming a kind and naming a type are one act. The validator still reads the kind locally off the URL, so the discriminated union stays local and strict. The older `ref` key is retired. <!-- id:UgN0v7lk -->
...
- **`anyOf` unions.** [`anyOf`](./anyof.md) was added so `schema` can be a discriminated union. It stays self-describing: `anyOf` is a list of schemas, expressible with the existing vocabulary, and the meta-schema validates as its own `union` variant. See [the schema language](./schema-language.md). <!-- id:KShLNAZM -->
- **`allOf` intersections merge.** [`allOf`](./allof.md) is `anyOf`'s _and_. Its arms are structs and they **merge** into one, the way extension merges over its parent, rather than each checking the value in turn: structs are closed by default, so two closed arms checked separately would accept nothing (the trap JSON Schema's `allOf` falls into). Merging also makes the intersection a subtype of every arm, which is what a typed reference needs. Arms that disagree on a field accept nothing and say so. Single-parent extension could express chains but not a type that is two things at once without copying fields, and a copy is not a subtype.
