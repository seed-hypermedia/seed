...
// compositeKeyStore wraps multiple KeyStores and checks them in order for reads.
type compositeKeyStore struct {
	primary KeyStore
	others  []KeyStore
}

// NewCompositeKeyStore creates a KeyStore that checks multiple stores in order.
// The first store is the "primary" where writes go by default.
// Reads check all stores in order until a key is found.
func NewCompositeKeyStore(primary KeyStore, others ...KeyStore) KeyStore {
...
func (c *compositeKeyStore) GetKey(ctx context.Context, name string) (*KeyPair, error) {
	if kp, err := c.primary.GetKey(ctx, name); err == nil {
		return kp, nil
	}
	for _, store := range c.others {
		if kp, err := store.GetKey(ctx, name); err == nil {
			return kp, nil
		}
	}
	return nil, fmt.Errorf("%s: %w", name, errKeyNotFound)
...
func (c *compositeKeyStore) StoreKey(ctx context.Context, name string, kp *KeyPair) error {
	return c.primary.StoreKey(ctx, name, kp)
}

func (c *compositeKeyStore) ListKeys(ctx context.Context) ([]NamedKey, error) {
	seen := make(map[string]bool)
	var result []NamedKey

	keys, err := c.primary.ListKeys(ctx)
	if err != nil {
		return nil, err
	}
	for _, k := range keys {
...
func (c *compositeKeyStore) DeleteKey(ctx context.Context, name string) error {
	if _, err := c.primary.GetKey(ctx, name); err == nil {
		return c.primary.DeleteKey(ctx, name)
	}
	for _, store := range c.others {
		if _, err := store.GetKey(ctx, name); err == nil {
			return store.DeleteKey(ctx, name)
		}
	}
	return errKeyNotFound
}

func (c *compositeKeyStore) DeleteAllKeys(ctx context.Context) error {
	if err := c.primary.DeleteAllKeys(ctx); err != nil {
		return err
	}
	for _, store := range c.others {
		if err := store.DeleteAllKeys(ctx); err != nil {
			return err
		}
	}
	return nil
...
func (c *compositeKeyStore) ChangeKeyName(ctx context.Context, currentName, newName string) error {
	if _, err := c.primary.GetKey(ctx, currentName); err == nil {
		return c.primary.ChangeKeyName(ctx, currentName, newName)
	}
	for _, store := range c.others {
		if _, err := store.GetKey(ctx, currentName); err == nil {
			return store.ChangeKeyName(ctx, currentName, newName)
		}
	}
	return errKeyNotFound
}
