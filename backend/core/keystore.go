...
// fileKeyStore stores keys as JSON files in a directory.
// This is insecure and should only be used for testing/fixtures.
type fileKeyStore struct {
	dir string
}

// NewFileKeyStore creates a key store backed by files in a directory.
// Keys are stored as plaintext JSON - only use for testing/fixtures.
...
func (fks *fileKeyStore) keysFilePath() string {
	return filepath.Join(fks.dir, "account_keys.json")
}

func (fks *fileKeyStore) readCollection() (keyCollection, error) {
...
func (fks *fileKeyStore) writeCollection(collection keyCollection) error {
