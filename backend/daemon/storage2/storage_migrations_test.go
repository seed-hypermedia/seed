...
func TestMigrateMatchesFreshSchema(t *testing.T) {
	...
	// If we need to break the compatibility, a new snapshot can be generated by running the following oneliner from the root of the repo:
	// `rm -rf backend/daemon/storage2/testdata/seed-test-db-snapshot && go run ./backend/cmd/mkdb && mv /tmp/seed-test-db-snapshot backend/daemon/storage2/testdata`
