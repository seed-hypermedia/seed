...
type Discoverer interface {
	// GetOrCreateEphemeralTask returns or creates a discovery task for the given parameters.
	// The task is ephemeral (evicts when not called) unless a subscription exists for the same IRI.
	GetOrCreateEphemeralTask(iri blob.IRI, version blob.Version, recursive bool) syncing.DiscoveryTaskInfo
}
...
func (api *Server) DiscoverEntity(ctx context.Context, in *entities.DiscoverEntityRequest) (*entities.DiscoverEntityResponse, error) {
	...
	info := api.disc.GetOrCreateEphemeralTask(iri, v, in.Recursive)

	resp := &entities.DiscoverEntityResponse{
		Version:   info.Result.String(),
		State:     stateToProto(info.State),
		CallCount: int32(info.CallCount), //nolint:gosec
