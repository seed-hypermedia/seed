	...
	if account := in.GetAccount(); account != "" {
		...
		query = `
			SELECT
				...
				latest_sb.ts
			FROM (
				SELECT sb.*, ROW_NUMBER() OVER (PARTITION BY sb.resource, sb.extra_attrs->>'tsid' ORDER BY sb.ts DESC) as rn
				FROM structural_blobs sb
				WHERE sb.type = 'Contact'
				AND sb.resource = (SELECT id FROM resources WHERE iri = 'hm://' || ?)
			) latest_sb
			LEFT JOIN resources res ON res.id = latest_sb.resource
			JOIN public_keys pk_account ON pk_account.id = COALESCE(res.owner, latest_sb.author)
			...
			LIMIT ?
		`
		args = []any{accountPrincipal.String(), cursor.ContactID, in.PageSize + 1}
	} else {
