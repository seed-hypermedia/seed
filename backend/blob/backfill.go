...
// wrong one here: an underived collection flag simply reads as "not a
// collection", the read path already falls back to the authored
// `"type": "Collection"` metadata, and new and updated documents get the field
// on the ordinary write path regardless. Only history needs filling in, and it
// can be filled in late without anyone noticing.
//
