...
// collection flag: whether the document's content is a single childless Query
// block listing the document's own children. Like FirstImageInContentAttr the
...
// be a collection" and falls back to the authored `"type": "Collection"`
// metadata, so a lagging backfill only ever costs coverage of legacy
...
func deriveDocFieldsForGeneration(conn *sqlite.Conn, bs *blockStore, log *zap.Logger, derive DeriveDocFields, k generationKey) (changesReplayed int, stored bool, err error) {
	...
	if derr != nil {
		...
		// is certainly not a single childless Query block.
		//
