...
func main() {
	...
	scanPeriod := flag.Duration("scan-period", time.Duration(10*time.Minute), "Space scanning periodicity.")
	peerTimeout := flag.Duration("peer-timeout", time.Duration(45*time.Second), "Timeout to consider the peer to be offline.")
	numPings := flag.Int("num-pings", 5, "Number of pings per space to calculate average TTR.")
