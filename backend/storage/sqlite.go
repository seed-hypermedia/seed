...
func OpenSQLite(uri string, flags sqlite.OpenFlags, poolSize int) (*sqlitex.Pool, error) {
	...
	return openSQLite(uri, flags, poolSize, writePrelude, readPrelude)
}

func openSQLite(uri string, flags sqlite.OpenFlags, poolSize int, writePrelude, readPrelude []string) (*sqlitex.Pool, error) {
...
func InitSQLiteSchema[T *sqlite.Conn | *sqlitex.Pool](db T) error {
	...
	case *sqlite.Conn:
		conn = v
	case *sqlitex.Pool:
		c, release, err := v.WriteConn(context.Background())
		if err != nil {
			return err
		}
		defer release()
