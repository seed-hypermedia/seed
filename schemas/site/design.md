...
instance of itself? ([the schema language](hm://z6MkmZUb4K5c17zGGBuJJerwFzBaGkiYLfEEnkb9CH1W1ptb/schema-language) shows the loop;
`validate.mjs` checks it.) Self-description is the design
constraint, not a party trick.

**Precise, not permissive.** The meta-schema is a *discriminated union* of seven
...
- **`integer` / `float` split.** DAG-CBOR encodes them differently; collapsing
  ...
  type. ([the data model](hm://z6MkmZUb4K5c17zGGBuJJerwFzBaGkiYLfEEnkb9CH1W1ptb/data-model))
...
- **Typed links via `ref`.** `type:"link"` + `ref` reuses an existing keyword
  ...
  is lazy by necessity. ([references](hm://z6MkmZUb4K5c17zGGBuJJerwFzBaGkiYLfEEnkb9CH1W1ptb/references))
- **References are names (`hm://` URLs), not CIDs.** A CID cycle has no encoding
  ...
  the earlier self-sentinel idea. ([references](hm://z6MkmZUb4K5c17zGGBuJJerwFzBaGkiYLfEEnkb9CH1W1ptb/references))
...
- **Extension by refinement.** `{ ref: parent, …refinements }` extends the
  ...
  Worked example: `example-employee` extends
  `example-person`. ([the schema language](hm://z6MkmZUb4K5c17zGGBuJJerwFzBaGkiYLfEEnkb9CH1W1ptb/schema-language))
- **`anyOf` unions.** Added so `schema` can be a discriminated union. It is the
  ...
  validates as its own `union` variant. ([the schema language](hm://z6MkmZUb4K5c17zGGBuJJerwFzBaGkiYLfEEnkb9CH1W1ptb/schema-language))
...
- **Generic abstraction (`params` / `var` / `args`).** Deferred until a real
  ...
  maps-of-schema on the existing variants. ([the schema language](hm://z6MkmZUb4K5c17zGGBuJJerwFzBaGkiYLfEEnkb9CH1W1ptb/schema-language))

## Open questions

**Value constraints beyond kind.** *Since added.* `minLength`/`maxLength`,
`pattern`, `minimum`/`maximum`, and `minItems`/`maxItems` — folded in from the
"Seed Blob Schema v1" dialect — now narrow the values a leaf accepts (see the
value-constraints section of [the schema language](hm://z6MkmZUb4K5c17zGGBuJJerwFzBaGkiYLfEEnkb9CH1W1ptb/schema-language)).
Each one is vocabulary the meta-schema must also be able to describe about
itself — cheap individually, but they accrete, so the bar for adding more (string
formats, etc.) stays high: add only with a real need.
...
([references](hm://z6MkmZUb4K5c17zGGBuJJerwFzBaGkiYLfEEnkb9CH1W1ptb/references)) but the bundle format is not specified. Left
