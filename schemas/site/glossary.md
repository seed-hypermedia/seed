...
exactly one kind. ([the data model](hm://z6MkmZUb4K5c17zGGBuJJerwFzBaGkiYLfEEnkb9CH1W1ptb/data-model))
...
is one of the meta-schema's seven variants.

**Meta-schema** — `onyx-schema`: the schema that describes what
a schema is. A **discriminated union** of seven variants; a valid instance of
...
"has `anyOf`" / "has `var`" / "bare `ref`"). Expressed with `anyOf`. ([the schema language](hm://z6MkmZUb4K5c17zGGBuJJerwFzBaGkiYLfEEnkb9CH1W1ptb/schema-language))

**Variant** — one of the seven member schemas of the meta-schema union:
`onyx-map-schema`, `onyx-list-schema`, `onyx-scalar-schema`, `onyx-link-schema`, `onyx-include-schema`,
`onyx-union-schema`, `onyx-var-schema`. Each is a closed map.
...
unbound. Worked example: `Change<Block>` (`hypermedia-change`)
instantiated as `example-myapp-change`.
([the schema language](hm://z6MkmZUb4K5c17zGGBuJJerwFzBaGkiYLfEEnkb9CH1W1ptb/schema-language))
...
content-addressed block for a kind; reference it (`{ "ref": "onyx-string" }`)
...
([the data model](hm://z6MkmZUb4K5c17zGGBuJJerwFzBaGkiYLfEEnkb9CH1W1ptb/data-model))
...
itself: `onyx-schema` matches its own `union` variant, whose `anyOf` items match
its `include` variant, whose targets match the other variants.
([the schema language](hm://z6MkmZUb4K5c17zGGBuJJerwFzBaGkiYLfEEnkb9CH1W1ptb/schema-language))

**Include** — a bare reference `{ "ref": "hm://…" }` (no `type`, no refinements).
Becomes exactly the referenced schema. ([references](hm://z6MkmZUb4K5c17zGGBuJJerwFzBaGkiYLfEEnkb9CH1W1ptb/references))
...
bare include. Example: `example-employee` extends
`example-person`.
([the schema language](hm://z6MkmZUb4K5c17zGGBuJJerwFzBaGkiYLfEEnkb9CH1W1ptb/schema-language))

**Link** — a value of kind `link`: a **CID** pointing to a separate block. A
*typed link* (`{ "type":"link", "ref":"x" }`) records the expected type of
...
([encoding](hm://z6MkmZUb4K5c17zGGBuJJerwFzBaGkiYLfEEnkb9CH1W1ptb/encoding))
...
filenames are the dev alias (`onyx-string` ⇄ `hm://hyper.media/string`).
...
see [the Hypermedia chapter](hm://z6MkmZUb4K5c17zGGBuJJerwFzBaGkiYLfEEnkb9CH1W1ptb/hypermedia)), and `hm://example.com/*` the examples.
...
([references](hm://z6MkmZUb4K5c17zGGBuJJerwFzBaGkiYLfEEnkb9CH1W1ptb/references))
