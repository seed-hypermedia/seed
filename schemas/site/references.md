...
`example-person`, `home` is `{ "ref": "example-address" }`: a
...
In `example-document`, `author` is a typed link to
`example-person`: the document block does not contain the person; it contains a
...
cannot confirm the target matches `example-person` without fetching that block. So
...
1. encodes each schema to DAG-CBOR (see [encoding](hm://z6MkmZUb4K5c17zGGBuJJerwFzBaGkiYLfEEnkb9CH1W1ptb/encoding)),
2. computes its CID,
3. rewrites every `ref` that named that file into the file's CID.

`{ "ref": "example-address" }` becomes `{ "ref": <cid-of-address-block> }`. Same
...
to itself — now through its variants. `onyx-schema` is `{ anyOf: [ …refs to the
variants… ] }`, and each variant (e.g. `onyx-map-schema`) contains `{ "ref":
"onyx-schema" }`. So `onyx-schema` → variant → `onyx-schema` is a **cycle**, and
...
`example-document` has `previous: { type: link, ref:
"example-document" }` — a document links to a previous document of the same type.
...
Now recursion just works. `example-folder`
references `hm://example.com/file`, and `example-file`
...
first; the names resolve lazily. (In the schema explorer you can click folder → file →
folder in a circle.)
...
(`onyx-string` ⇄ `hm://hyper.media/string`, `example-file` ⇄
