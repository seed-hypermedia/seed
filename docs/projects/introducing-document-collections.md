---
name: 'Introducing Document Collections'
summary:
  'Make collections a structural document type that stays correct across drafts, publishing, and file-browser listings.'
---

# Introducing Document Collections
...
- The file browser could not identify published collections from typed listing data without inspecting metadata or fetching
  full documents.
- Local conversions could look correct on the document page but lose their collection icon after navigation because the
...
The product model has also become simpler: the feature is called a **Collection**. A Collection is
still a document. Its behavior comes from its content, not from a user-authored type flag.

## Solution

### Define Collection identity from document content
...
The shared document machine derives `documentType: 'document' | 'collection'` from effective content:
...
The document page reads Collection status directly from machine selectors. React does not inspect metadata, manufacture
Collection state, coordinate repair effects, or write drafts directly.

For now, every Collection renders with the shared Table view, even when its stored query asks for Card or List. We preserve
the stored presentation so additional views can be supported later without changing Collection identity.

### Make conversion explicit and draft-first

- **Convert to Collection** asks for confirmation because it removes the document's existing top-level content. Confirming
  replaces the effective content with one canonical self-query block and enters the normal editing flow.
- **Convert to Document** asks for confirmation because it removes the Collection query. Confirming leaves an empty document
...
Add **New Collection** beside **New Document** on desktop and web. A new Collection starts as a normal public draft with empty
...
The file browser shows the Collection icon immediately, including before publication and after navigating away or reloading.

### Derive at every persistence boundary

Runtime state and persisted indexes have different lifetimes, so each persistence boundary derives Collection status from
...
- Web stores `isCollection` in each IndexedDB draft record.
- The backend stores `$db.isCollection` as an internal derived attribute for each published document generation.
...
### Expose published Collection status as typed listing data

`DocumentInfo.is_collection` exposes the optional backend-derived value without polluting user metadata. The indexer derives
it from the merged published document heads and overwrites both `true` and `false`, so converting a Collection back into a
...
- As an author, I can create a Collection on desktop or web and immediately see a Table view and Collection icon.
- As an author, I can navigate away from an unpublished Collection and still see its correct icon in the file browser.
- As an editor, I can convert a populated document to a Collection only after confirming that its content will be removed.
- As an editor, I can convert a Collection to an empty document only after confirming that its query will be removed.
- As an editor, I can cancel either conversion without changing the draft.
- As a reader, I see published Collection icons from lightweight document listing data without fetching every document.
...
- A one-query document remains a Collection regardless of query presentation or nested children.
...
- Add the shared structural predicate and canonical Collection query seed.
...
- Expose direct selectors for document type and the Collection query block.

### Phase 2: Draft persistence and creation

- Persist derived `isCollection` values in desktop JSON and web IndexedDB draft records.
- Keep legacy draft records readable with a normal-document default.
- Add New Collection creation on desktop and web using the same canonical seed.
- Preserve query retargeting when a new Collection is published at its final location.

### Phase 3: Rendering and file-browser integration

- Render Collections through the shared Table view without making Table part of their identity.
- Add confirmed, toast-free conversion actions.
- Overlay matching local draft status on published directory rows.
- Render unpublished new Collections and conversions correctly across navigation and reloads.

### Phase 4: Published index and rollout

- Derive Collection status from the real Go document model during incremental indexing.
- Expose optional `DocumentInfo.is_collection` listing data.
...
- Supporting Card, List, gallery, kanban, calendar, or user-selectable Collection rendering in the first version.
- Persisting query search and ad hoc filters as part of Collection identity.
...
- Adding a separate published document-type enum when listings only need the derived Collection fact.
...
- Do not use authored metadata to decide whether a document is a Collection.
- Do not trust a caller-provided Collection flag at draft or backend write boundaries; derive it from content.
- Do not require Table presentation for Collection identity or rewrite Card/List queries merely because they are Collections.
- Do not create repair drafts when a Collection is opened.
- Do not publish conversions automatically or bypass the normal draft review and Publish flow.
- Do not let React coordinate conversion state, directly write Collection drafts, or independently reproduce the predicate.
- Do not show conversion success, Undo, or automatic-repair toasts.
- Do not use a blocking migration or migration-triggered full reindex to backfill Collection status.
