# Hypermedia Schemas schema model v2 — corrected schema bindings
...
The sync publishes, under the Hypermedia account:
- the schema DAG-CBOR blob (CID verified vs `schemas.lock.json`);
- a document at `hm://<library>/<name>` whose **content** is the `.md` and whose
  metadata **`schemaDefinition`** = `ipfs://<schema CID>`.

So `hm://<library>/example-person` becomes the referenceable "person" schema URL.
...
2. **Publishing** — co-locate `<name>.md` beside every `<name>.json`; update the
   sync to publish each pair as `hm://<library>/<name>` with `schemaDefinition` = CID.
...
`resource-page-common.tsx`, `hypermedia/schema-document.tsx`, `schema/metadata-schema-keys.ts`.
