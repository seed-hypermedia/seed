# Hypermedia Schemas build progress

**Watch this file** for live progress. Updated as I go. Status: ✅ done · 🚧 in progress · ⛔ blocked · ⬜ todo.

Branch: the second schemas branch (off `feat/schemas`). Reference system: `schemas/` (Hypermedia Schemas) + tour.

---

## 0 · Current state (done)

- ✅ Hypermedia Schemas type system + `validate.mjs` (257 checks) + `publish.mjs` (deterministic CIDs → `schemas.lock.json`) + `tour.mjs` explorer.
- ✅ Full schemafication of the six CBOR blob types, block model (strict core + open + extension), `Change<Block>` generics.
- ✅ Integration roadmap: [`schemas-integration-plan.md`](./schemas-integration-plan.md).

---

## 1 · ✅ DECIDED — Hypermedia Schemas canonical, absorb v1  *(2026-07-13)*

**Hypermedia Schemas is the canonical base.** Fold v1's best ideas into it — value constraints
...
`blob-schema-editor` to be Hypermedia Schemas-backed** (reuse its UX; don't rewrite). Hypermedia Schemas keeps
...
### (reference) Hypermedia Schemas vs. "Seed Blob Schema v1"
...
an 889-line `blob-schema-editor.tsx`). It overlaps heavily with Hypermedia Schemas. We must pick a
direction before building editors, because the editor's form logic depends on the dialect.

| | **Hypermedia Schemas** (this session) | **Seed Blob Schema v1** (feat/schemas) |
| --- | --- | --- |
...
replies; the change→op→block stack; documents referencing documents). the schema language's **name-refs**
...
**Recommendation:** make **Hypermedia Schemas the canonical base** (its recursion + generics + self-hosting
...
`blob-schema-editor` to be Hypermedia Schemas-backed** (reuse its UX, don't rewrite from scratch). This
matches the stated goal ("integrate Hypermedia Schemas", "replace hm-types with Hypermedia Schemas").
...
### ▶ Iteration (2026-07-13): Inspector & Explorer integrated with the editor/viewer + Hypermedia Schemas
User: *"bring in the inspector and explorer features to support hypermedia and be fully integrated with the new
...
- ✅ **Phase 1 — Document/IPFS inspector recognizes hypermedia (Hypermedia Schemas) blob types** (`417985a5f`). New pure
  ...
  never matches). "Hypermedia Schemas: <Type>" badge in the IPFS inspector. +tests. (Document `inspector-page.tsx` skipped
...
- ✅ **Phase 3 — Hypermedia Schemas absorbs v1's value constraints + advisory validation** (`a9c0e8164`) — the reconciliation's
  ...
  Meta-schema variants (`hypermedia-scalar-schema`/`hypermedia-list-schema`) extended to permit + self-validate the
...
schemas validator clean. Remaining reconciliation (evolve the app editor to be Hypermedia Schemas-backed; `@shm/hypermedia` TS
...
- ⬜ **Package** — extract the validator/editor into `@shm/hypermedia` (or reconcile with `@shm/ui`).
...
- ⬜ Protocol/daemon/RPC phases — per [`schemas-integration-plan.md`](./schemas-integration-plan.md).

---

## 3 · Log

- **2026-07-13** — Cherry-picked 10 Hypermedia Schemas commits onto `feat/schemas`; created the second schemas branch. Wrote the
  integration roadmap. Researched the frontend: found **Seed Blob Schema v1** (existing in-app
  schema system + editor). Raised the blocking Hypermedia Schemas-vs-v1 decision (§1). Build paused on it.
