# Hypermedia Schemas integration plan

Bringing Hypermedia Schemas (the self-describing type system in `schemas/`) deep into the Seed
Hypermedia protocol and apps.

## North star

Hypermedia Schemas becomes the **single source of truth for Seed's data types**: schemas
...
2. **Hypermedia Schemas validates; it does not coerce.** Zod does defaults/transforms; Hypermedia Schemas
...
- `frontend/packages/hypermedia` (`@shm/hypermedia`): port `validate.mjs` → TS
...
**Deliverable:** `import { validate } from "@shm/hypermedia"`; conformance suite green in CI.
...
**Deliverable:** Hypermedia Schemas validates 100% of a real blob corpus; a report of schema fixes.

## Phase 2 — Codegen: Hypermedia Schemas → TypeScript

- Generator in `@shm/hypermedia`: schemas → `hm-types.generated.ts`.
...
## Phase 3 — Gradual replacement of `hm-types.ts` (Zod → Hypermedia Schemas)
...
- **3b** Differential test: each Zod schema vs. its Hypermedia schema on shared fixtures.
- **3c** Replace Zod `.parse` with Hypermedia Schemas `validate`+`parse`, one type at a time.
...
- Handle Zod-only features (defaults, `transform`, coercion — e.g. `HMTimestamp`
  ...
  `@shm/hypermedia`, or keep thin Zod adapters at the edges.

**Deliverable:** block/blob/metadata types in `hm-types.ts` are Hypermedia Schemas-backed.
...
- Port Hypermedia Schemas to Go — the spec is small (~200 lines: kinds, closed maps, unions,
...
**Deliverable:** `hypermedia.Validate` in Go passing the corpus; daemon warn-validates.

## Phase 6 — RPC typing

- The API is gRPC/proto (`backend/genproto`). Options: validate JSON/RPC payloads
  against Hypermedia schemas at the gateway; and/or generate Hypermedia schemas from (or
  alongside) proto. Start with the JSON API surface.

**Deliverable:** one RPC method's request/response Hypermedia Schemas-validated end to end.
...
- Unify with the **existing schema UI already on `feat/schemas`** (developer mode,
  schema-typed metadata fields, HM URL / Profile schema types): Hypermedia Schemas becomes the
...
## Suggested first slice (second schemas branch)

The smallest sequence that proves the whole thesis before touching the protocol:

1. **Phase 0** — `@shm/hypermedia` (TS validator + bundled schemas) + conformance corpus.
...
That demonstrates *Hypermedia Schemas validates real production data AND generates the TS types*,
