...
async function cmdDoctor(
  ...
  if (config) {
    ...
    const webEnvJson = shell.runSafe(
      "docker inspect seed-web --format '{{json .Config.Env}}' 2>/dev/null",
    );
    let webPort = "3000";
    if (webEnvJson) {
    ...
    if (!siteConfigRaw) {
      ...
      try {
        ...
        if (accountUid) {
          const shortUid =
            accountUid.length > 16
              ? `${accountUid.slice(0, 8)}...${accountUid.slice(-6)}`
              : accountUid;
          console.log(`  \u2714 Site           registered (${shortUid})`);
        } else {
          console.log(`  \u26A0 Site           not registered`);
          console.log(
            "      Tip: On the Seed app, use the autogenerated secret key to connect this site.",
          );
          console.log(`      Run '${cmd("secret")}' to view the key.`);
        }
      } catch {
        console.log(`  \u26A0 Site           could not check`);
      }
...
async function cmdSecret(paths: DeployPaths): Promise<void> {
  ...
  console.log(config.link_secret);
}
