...
import { HM_DIR, LOCK_PATH, listSchemaFiles, nameOfFile, nameToUrl } from "./names.mjs";
...
if (process.argv.includes("--check")) {
  ...
  if (current !== rendered) {
    console.error("schemas.lock.json is out of date — run `node scripts/hypermedia/publish.mjs` and commit.");
    process.exit(1);
  }
