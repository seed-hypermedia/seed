...
function urlToBasename(url) {
  ...
  if (!m) return url
  const [, auth, name] = m
  if (auth !== HYPERMEDIA_UID && auth !== 'hyper.media') return null
...
function emitSchema(basename) {
  ...
  docLines.push(`Schema: hm://${HYPERMEDIA_UID}/${basename}`)
  const doc = `/**\n * ${docLines.join('\n * ').replace(/\*\//g, '*\\/')}\n */`
...
// content-addressed schemas published under hm://${HYPERMEDIA_UID}.
