...
const base = (n) => n.replace(/\.schema\.json$|\.(json|md)$/, "");
const files = (() => {
  const out = [];
  const walk = (d, prefix) => {
    for (const entry of readdirSync(d).sort()) {
      ...
      else if (entry.endsWith(".md") || entry.endsWith(".json")) out.push(`${prefix}${entry}`);
    }
  };
  walk(DIR, "");
  return out;
...
const urlToFile = (ref) => fileOfName(refToName(ref, (n) => SCHEMA_FILES.includes(fileOfName(n))));
...
// Attribute-safe encoding (for JSON seeds carried in data-* attributes).
...
function schemaPage(name) {
  ...
  const body = `
    ...
    ${depLists(name)}
    <section class="editor-wrap">
      <h2>${isMeta ? "Schema editor" : "Data editor"} <span class="muted">· live, validated by the ported engine</span></h2>
...
try {
  const out = execFileSync("node", ["validate.mjs"], { cwd: SCRIPT_DIR, encoding: "utf8" });
...
/* editor */
.editor-wrap{margin:30px 0}
.on-grid{display:grid;grid-template-columns:1fr 1fr;gap:16px;align-items:start}
.on-col-title{font-size:11px;text-transform:uppercase;letter-spacing:.1em;color:var(--dim);margin-bottom:10px}
.on-form-col,.on-out-col{background:var(--panel);border:1px solid var(--line);border-radius:14px;padding:16px 18px;min-width:0}
.on-out-col{position:sticky;top:18px}
.on-form{display:flex;flex-direction:column;gap:10px}
.on-field{border:1px solid var(--line);border-radius:10px;padding:10px 12px;background:var(--panel2)}
.on-field-head{display:flex;align-items:center;gap:8px;margin-bottom:7px}
.on-key{font-family:"SF Mono",monospace;font-size:13px;color:#9ecbff}
.on-req{font-size:10.5px;color:#e0a15e;margin-left:auto}
.on-opt{font-size:10.5px;color:var(--muted);margin-left:auto}
.on-include{accent-color:var(--accent)}
.on-field-body{padding-left:2px}
.on-input,.on-select,.on-textarea{width:100%;background:var(--bg);border:1px solid var(--line);border-radius:8px;color:var(--ink);
  font:13px "SF Mono",ui-monospace,monospace;padding:7px 9px}
.on-input:focus,.on-select:focus,.on-textarea:focus{outline:none;border-color:var(--accent)}
.on-select{cursor:pointer}
.on-textarea{resize:vertical;line-height:1.5}
.on-textarea.bad{border-color:#f3849b}
.on-bool{display:inline-flex;align-items:center;gap:8px;cursor:pointer}
.on-bool-txt{font-family:"SF Mono",monospace;font-size:12px;color:var(--kind-boolean)}
.on-null{font-family:"SF Mono",monospace;color:var(--kind-null);font-size:13px}
.on-wrapped{display:flex;align-items:center;gap:8px;flex-wrap:wrap}
.on-wrapped .on-input{flex:1;min-width:120px}
.on-envelope{font-size:11px;padding:2px 8px;border-radius:6px;white-space:nowrap;font-family:"SF Mono",monospace}
.on-envelope.link{color:var(--kind-link);background:#2a1620;border:1px solid #6b2c3a}
.on-envelope.bytes{color:var(--kind-bytes);background:#20182a;border:1px solid #3a2f5a}
.on-union{border-left:2px solid var(--accent2);padding-left:10px}
.on-union-slot{margin-top:8px}
.on-list-items,.on-extras{display:flex;flex-direction:column;gap:8px;margin-bottom:8px}
.on-item,.on-entry{display:flex;align-items:flex-start;gap:8px}
.on-item>*:nth-child(2),.on-entry>*:nth-child(2){flex:1;min-width:0}
.on-idx{font-family:"SF Mono",monospace;font-size:11px;color:var(--muted);padding-top:8px;min-width:30px}
.on-keyinput{max-width:160px}
.on-open-note{font-size:11px;color:var(--muted);margin:4px 0}
.on-hint{font-size:11px;color:var(--muted);margin-bottom:4px}
.on-btn{background:var(--panel2);border:1px solid var(--line);color:var(--dim);border-radius:8px;padding:5px 11px;
  font:12px -apple-system,sans-serif;cursor:pointer}
.on-btn:hover{border-color:var(--accent);color:var(--ink)}
.on-add{color:#8bd17c;border-color:#2f5a34}
.on-rm{color:var(--muted);padding:5px 9px}
.on-rm:hover{color:#f3849b;border-color:#6b2c3a}
.on-status{font-size:13px;padding:8px 12px;border-radius:9px;border:1px solid var(--line);margin-bottom:12px}
.on-status.ok{color:#8be2a1;border-color:#2c6b3f;background:#12201a}
.on-status.bad{color:#f3b7c4;border-color:#6b2c3a;background:#2a1620}
.on-dot{display:inline-block;width:8px;height:8px;border-radius:50%;margin-right:7px}
.on-dot.ok{background:#5fd67a;box-shadow:0 0 8px #5fd67a88}
.on-dot.bad{background:#f3849b}
.on-errs{margin:8px 0 0;padding-left:18px;font-family:"SF Mono",monospace;font-size:11.5px;line-height:1.6}
.on-out{margin:0;max-height:460px}
.on-bar{display:flex;gap:8px;margin-top:12px}
.on-copy{color:var(--accent)}
