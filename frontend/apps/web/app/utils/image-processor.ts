...
export async function _processImage(imageCid: string): Promise<string> {
  ...
  try {
    ...
    const mimeType = contentType.includes('image/') ? contentType : `image/${format}`
  ...
  return (input: Input) => {
    return Promise.race([
      ...
      new Promise<PromiseResult>((_, reject) => {
        setTimeout(() => reject(new Error(`Timeout: ${label} (${timeout}ms)`)), timeout)
      }),
    ])
  }
}
...
export const processImage = withTimeout(_processImage, 'generating image for author', 4_000)
