...
import {Capability, hmId, Role} from '@shm/shared'
...
export const action = cborApiAction<EmailNotifierAction, any>(
  async (signedPayload, {pathParts}) => {
    ...
    if (signerId !== accountId) {
      const caps = await grpcClient.accessControl.listCapabilitiesForDelegate({
        delegate: signerId,
      })
      const agentCap = caps.capabilities.find(
        (cap: PlainMessage<Capability>) =>
          cap.role === Role.AGENT && cap.issuer === accountId,
      )
      if (!agentCap) {
