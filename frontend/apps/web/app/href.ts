...
export function getHref(homeId: UnpackedHypermediaId | null | undefined, id: UnpackedHypermediaId, version?: string) {
  ...
  const fragment = id.blockRef ? `#${id.blockRef}${serializeBlockRange(id.blockRange)}` : ''
  let urlWithoutHost = `/hm/${id.uid}${path}${version ? `?v=${version}` : ''}${fragment}`
  if (homeId && homeId.uid === id.uid) urlWithoutHost = `${path}${version ? `?v=${version}` : ''}${fragment}`
  if (id.hostname) {
