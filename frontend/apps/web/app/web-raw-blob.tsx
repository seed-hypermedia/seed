...
import type {RawBlobRoute} from '@shm/shared/routes'
...
export function ipfsUrlToRoute(url: string): {key: 'raw-blob'; cid: string} | null {
  const cidText = url.trim().replace(/^ipfs:\/\//, '')
  return parseCidString(cidText) ? {key: 'raw-blob', cid: cidText} : null
}

/**
 * Parse the reserved `/hm/blob/…` gateway URL path (split into segments) into a
 * raw-blob nav route — the inverse of `routeToHref`'s raw-blob case:
 *   /hm/blob/ipfs/<cid>            → edit an existing blob
 *   /hm/blob/new-instance/<schCid> → new instance seeded by that schema
 *   /hm/blob/new                   → a blank new blob
 * Returns null for any other path (so the site router falls through to it).
 */
export function extractRawBlobRouteFromPath(
  pathParts: string[],
): {key: 'raw-blob'; cid?: string; schemaCid?: string} | null {
  if (pathParts[0] !== 'hm' || pathParts[1] !== 'blob') return null
  const rest = pathParts.slice(2)
  if (rest[0] === 'ipfs' && rest[1]) return {key: 'raw-blob', cid: rest.slice(1).join('/')}
  if (rest[0] === 'new-instance' && rest[1]) return {key: 'raw-blob', schemaCid: rest.slice(1).join('/')}
  if (rest[0] === 'new') return {key: 'raw-blob'}
  return null
}
 ...
 */
export function blobBuilderMenuItems(navigate: (route: RawBlobRoute) => void): MenuItemType[] {
  return [
    {
      key: 'new-raw-blob',
      label: 'New Blob',
...
/** Full-page GUI editor for raw DAG-CBOR IPFS blobs (web). */
export function WebRawBlobPage() {
  const route = useNavRoute()
  if (route.key !== 'raw-blob') {
    throw new Error(`WebRawBlobPage: unsupported route ${route.key}`)
  }
  return (
