/**
 * Hypermedia Schemas for the CLI: load a schema by any reference, validate values, encode blobs, and check
...
import {
  ...
  resolveSchema,
  validate,
} from '@seed-hypermedia/client/schema-engine'
import {
...
export async function loadSchema(client: SeedClient, ref: string): Promise<LoadedSchema> {
  ...
  if (/\.json$/i.test(ref) && existsSync(resolvePath(ref))) {
    ...
    schema = parsed as HypermediaSchema
    cid = (await encodeBlob(schema)).cid
    source = `file ${ref}`
  } else if (HM_SCHEMAS[ref]) {
    schema = HM_SCHEMAS[ref]!
    source = `library ${ref}`
  } else {
...
export function violations(schema: HypermediaSchema, value: unknown, registry: SchemaRegistry = {}): string[] {
...
export function describeType(schema: HypermediaSchema, registry: SchemaRegistry): string {
