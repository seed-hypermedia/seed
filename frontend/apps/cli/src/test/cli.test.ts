...
describe('Seed CLI Integration Tests', () => {
  ...
  describe('Account Commands', () => {
    test('accounts list shows registered accounts', async () => {
    ...
    test('account <uid> shows account info', async () => {
      const result = await runCli(['account', account1.accountId], {server: ctx.daemonUrl})
      expect(result.exitCode).toBe(0)
      expect(result.stdout).toContain(account1.accountId)
    ...
    test('account with --pretty shows formatted output', async () => {
      const result = await runCli(['account', account1.accountId, '--pretty'], {server: ctx.daemonUrl})
      expect(result.exitCode).toBe(0)
      expect(result.stdout).toContain('Test Account Alpha')
    }, TEST_TIMEOUT)
  })
  ...
  describe('Document Commands', () => {
    test('get account home document', async () => {
      const result = await runCli(['get', `hm://${account1.accountId}`], {server: ctx.daemonUrl})
      expect(result.exitCode).toBe(0)
      expect(result.stdout).toContain('document')
    }, TEST_TIMEOUT)

    test('get with --md outputs markdown', async () => {
      const result = await runCli(['get', `hm://${account1.accountId}`, '--md'], {server: ctx.daemonUrl})
      expect(result.exitCode).toBe(0)
      expect(result.stdout).toContain('# Test Account Alpha')
    }, TEST_TIMEOUT)
    ...
    test('get -q shows minimal output', async () => {
      const result = await runCli(['get', `hm://${account1.accountId}`, '-q'], {server: ctx.daemonUrl})
      expect(result.exitCode).toBe(0)
    ...
    test('get -m fetches metadata only', async () => {
      const result = await runCli(['get', `hm://${account1.accountId}`, '-m'], {server: ctx.daemonUrl})
      expect(result.exitCode).toBe(0)
      expect(result.stdout).toContain('Test Account Alpha')
    ...
    test('get non-existent document returns error', async () => {
      const result = await runCli(['get', 'hm://z6MknonexistentAAAAAAAAAAAAAAAAAAAA/test'], {server: ctx.daemonUrl})
  ...
  describe('Search Commands', () => {
    test('search finds accounts by name', async () => {
      const result = await runCli(['search', 'Alpha'], {server: ctx.daemonUrl})
      expect(result.exitCode).toBe(0)
      expect(result.stdout).toContain('Test Account Alpha')
    }, TEST_TIMEOUT)

    test('search -q shows compact output', async () => {
      const result = await runCli(['search', 'Test Account', '-q'], {server: ctx.daemonUrl})
      expect(result.exitCode).toBe(0)
      const lines = result.stdout.split('\n').filter(Boolean)
    ...
    test('search with no results', async () => {
      const result = await runCli(['search', 'xyznonexistent123'], {server: ctx.daemonUrl})
      expect(result.exitCode).toBe(0)
    }, TEST_TIMEOUT)
  })
  ...
  describe('Query Commands', () => {
    test('query lists documents in space', async () => {
      const result = await runCli(['query', account1.accountId], {server: ctx.daemonUrl})
      expect(result.exitCode).toBe(0)
    }, TEST_TIMEOUT)

    test('children lists child documents', async () => {
      const result = await runCli(['children', account1.accountId], {server: ctx.daemonUrl})
      expect(result.exitCode).toBe(0)
    }, TEST_TIMEOUT)

    test('query with --yaml outputs yaml', async () => {
      const result = await runCli(['query', account1.accountId, '--yaml'], {server: ctx.daemonUrl})
      expect(result.exitCode).toBe(0)
  ...
  describe('Comments Commands', () => {
    test('comments on document (empty)', async () => {
      const result = await runCli(['comments', `hm://${account1.accountId}`], {server: ctx.daemonUrl})
      expect(result.exitCode).toBe(0)
    }, TEST_TIMEOUT)

    test('discussions on document', async () => {
      const result = await runCli(['discussions', `hm://${account1.accountId}`], {server: ctx.daemonUrl})
      expect(result.exitCode).toBe(0)
    }, TEST_TIMEOUT)
  })
  ...
  describe('Changes Commands', () => {
    test('changes shows document history', async () => {
      const result = await runCli(['changes', `hm://${account1.accountId}`], {server: ctx.daemonUrl})
      expect(result.exitCode).toBe(0)
      expect(result.stdout).toContain('changes')
    }, TEST_TIMEOUT)

    test('changes -q shows compact output', async () => {
      const result = await runCli(['changes', `hm://${account1.accountId}`, '-q'], {server: ctx.daemonUrl})
      expect(result.exitCode).toBe(0)
  ...
  describe('Capabilities Commands', () => {
    test('capabilities shows access control', async () => {
      const result = await runCli(['capabilities', `hm://${account1.accountId}`], {server: ctx.daemonUrl})
      expect(result.exitCode).toBe(0)
    }, TEST_TIMEOUT)
  })
  ...
  describe('Activity Commands', () => {
    ...
    test('activity with --limit', async () => {
      const result = await runCli(['activity', '--limit', '5'], {server: ctx.daemonUrl})
      expect(result.exitCode).toBe(0)
    }, TEST_TIMEOUT)
  })
  ...
  describe('Stats Commands', () => {
    test('stats shows interaction summary', async () => {
      const result = await runCli(['stats', `hm://${account1.accountId}`], {server: ctx.daemonUrl})
      expect(result.exitCode).toBe(0)
  ...
  describe('Key Management Commands', () => {
    test('key generate creates new key', async () => {
      const result = await runCli(['key', 'generate', '-n', 'test-key'], {server: ctx.daemonUrl})
      expect(result.exitCode).toBe(0)
      expect(result.stdout).toContain('test-key')
      expect(result.stdout).toContain('created')
    }, TEST_TIMEOUT)

    test('key list shows stored keys', async () => {
    ...
    test('key derive computes account id', async () => {
      ...
      const result = await runCli(['key', 'derive', mnemonic], {server: ctx.daemonUrl})
      expect(result.exitCode).toBe(0)
  ...
  describe('Output Formats', () => {
    test('--json outputs valid JSON', async () => {
      const result = await runCli(['accounts', '--json'], {server: ctx.daemonUrl})
      expect(result.exitCode).toBe(0)
      expect(() => JSON.parse(result.stdout)).not.toThrow()
    }, TEST_TIMEOUT)

    test('--yaml outputs YAML format', async () => {
      const result = await runCli(['account', account1.accountId, '--yaml'], {server: ctx.daemonUrl})
      expect(result.exitCode).toBe(0)
  ...
  describe('Error Handling', () => {
    ...
    test('invalid server url handles gracefully', async () => {
      const result = await runCli(['accounts'], {server: 'http://localhost:99999'})
      expect(result.exitCode).toBe(1)
    }, TEST_TIMEOUT)
  })
  ...
  describe('Cross-Account Workflows', () => {
    test('both accounts visible in accounts list', async () => {
      const result = await runCli(['accounts', '-q'], {server: ctx.daemonUrl})
      expect(result.exitCode).toBe(0)
      expect(result.stdout).toContain(account1.accountId)
      expect(result.stdout).toContain(account2.accountId)
    ...
    test('can fetch both account home documents', async () => {
      const result1 = await runCli(['get', `hm://${account1.accountId}`, '-q'], {server: ctx.daemonUrl})
      const result2 = await runCli(['get', `hm://${account2.accountId}`, '-q'], {server: ctx.daemonUrl})

      expect(result1.exitCode).toBe(0)
      expect(result2.exitCode).toBe(0)
    }, TEST_TIMEOUT)

    test('search finds both accounts', async () => {
      const result = await runCli(['search', 'Test Account'], {server: ctx.daemonUrl})
      expect(result.exitCode).toBe(0)
