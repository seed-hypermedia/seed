...
import {loader, action} from './routes/hm.api.notification-inbox'

describe('hm.api.notification-inbox route', () => {
  it('returns a CORS preflight response for OPTIONS requests', async () => {
    const response = asResponse(
      await loader(
        createLoaderArgs(
          new Request('http://localhost/hm/api/notification-inbox', {
            method: 'OPTIONS',
          }),
        ),
      ),
