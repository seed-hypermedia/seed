...
describe('notification-config signing flow', () => {
  ...
  it('full round-trip: sign, verify, store, retrieve config', async () => {
    ...
    const setValid = await validateSignature(
      signer.compressedKey,
      setSig,
      new Uint8Array(setEncoded),
    )
    ...
    const getValid = await validateSignature(
      signer.compressedKey,
      getSig,
      new Uint8Array(getEncoded),
    )
