...
program
  .requiredOption(
    '-c, --current <path>',
    'Path to current performance metrics JSON file',
  )
  .requiredOption(
    '-b, --baseline <path>',
    'Path to baseline performance metrics JSON file',
  )
  .requiredOption(
    '-o, --output <path>',
    'Output path for comparison report HTML',
  )
  .option(
    '-t, --threshold <percent>',
    'Percentage threshold for highlighting significant changes',
    '5',
  )
...
function compareMetrics(
  ...
  for (const scenario of allScenarios) {
    ...
    Object.keys(baselineMetrics).forEach((key) => {
      if (
        typeof baselineMetrics[key] === 'object' &&
        baselineMetrics[key] !== null
      ) {
    ...
    Object.keys(currentMetrics).forEach((key) => {
      if (
        typeof currentMetrics[key] === 'object' &&
...
<body>
  ...
  <div class="summary">
    ...
    <div class="summary-box">
      <div class="summary-number improvement-number">${
        improvements.length
      }</div>
...
function generateAllMetricsRows(
  ...
  for (const scenario of allScenarios) {
    ...
    for (const metric of getAllMetricsInObject(
      baselineMetrics,
      currentMetrics,
    )) {
      let baselineValue: number | undefined
      ...
      rows.push(`
        <tr class="${
          Math.abs(percentChange) >= parseFloat(options.threshold)
            ? improved
              ? 'improvement'
              : 'regression'
            : ''
    ...
    try {
      if (fs.statSync(options.baseline).isDirectory()) {
        ...
        const baselineFiles = fs
          .readdirSync(options.baseline)
          .filter(
            (file) =>
              file.endsWith('.json') && file.startsWith('perf-metrics-'),
          )
          .map((file) => path.join(options.baseline, file))

        if (baselineFiles.length === 0) {
          throw new Error(
            `No performance metrics JSON files found in ${options.baseline}`,
          )
        }
    ...
    try {
      if (fs.statSync(options.current).isDirectory()) {
        ...
        if (currentFiles.length === 0) {
          throw new Error(
            `No performance metrics JSON files found in ${options.current}`,
          )
        }
