...
const best_practices_checker_1 = require("./best-practices-checker");
const perf_utils_1 = require("./perf-utils");
const performance_budgets_1 = require("./performance-budgets");
...
commander_1.program
    ...
    .version("0.1.0");
commander_1.program
    .option("-s, --scenarios <scenarios>", "Comma-separated list of scenarios to run (default: all)", "all")
    .option("-o, --output-dir <dir>", "Output directory for test results", "results")
    ...
    .option("--url <url>", "URL to run Lighthouse audit against (required for lighthouse option)", "http://localhost:9222")
    ...
    .option("--fail-on-budget-error", "Exit with error code if budget error violations are found", false);
...
async function main() {
    ...
    let performanceBudgets = performance_budgets_1.defaultBudgets;
    if (options.budgetFile) {
        try {
            performanceBudgets = (0, performance_budgets_1.loadBudgetsFromFile)(options.budgetFile);
            console.log(`📋 Loaded custom performance budgets from ${options.budgetFile}`);
        }
        catch (error) {
    ...
    results[scenarios_1.appStartupScenario.name] = await (0, perf_utils_1.measureAppStartupTime)();
    // Run each scenario
    for (const scenario of scenariosToRun) {
        if (scenario.name === scenarios_1.appStartupScenario.name) {
            continue; // Already measured
        }
        console.log(`📊 Running scenario: ${scenario.name}`);
        try {
            results[scenario.name] = await (0, perf_utils_1.runPerformanceScenario)(scenario);
    ...
    if (options.lighthouse) {
        ...
        try {
            ...
            console.log("NOTE: For Electron apps, the URL should be the actual web server URL where your app content is served.");
            console.log("      The URL 'localhost:9222' is typically just the Chrome debugging port, not your actual app.");
            console.log("      For Vite-based apps, try 'http://localhost:5173' instead.");
            // Add retry logic for Lighthouse
            let retries = 3;
            let lighthouseMetrics = {};
            while (retries > 0) {
                try {
                    lighthouseMetrics = await (0, perf_utils_1.runLighthouseAudit)(urlToAudit);
    ...
    if (options.bestPractices) {
        ...
        try {
            ...
            const bestPracticesResults = await (0, best_practices_checker_1.runBestPracticeChecks)(app, appWindow);
            // Generate best practices report
            const bestPracticesReportPath = path.join(outputDir, "best-practices-report.html");
            (0, best_practices_checker_1.generateBestPracticesReport)(bestPracticesResults, bestPracticesReportPath);
            console.log(`✅ Best practices report generated at: ${bestPracticesReportPath}`);
            await app.close();
        }
        catch (error) {
    ...
    // Save results
    const resultsPath = await (0, perf_utils_1.saveMetricsToJson)(results, outputDir);
    console.log(`💾 Results saved to: ${resultsPath}`);
    // Check performance against budgets if requested
    if (options.budget) {
        console.log("📋 Checking performance against budgets...");
        try {
            const budgetResults = (0, performance_budgets_1.checkPerformanceBudgets)(results, performanceBudgets);
            // Generate budget report
            const budgetReportPath = path.join(outputDir, "budget-report.html");
            (0, performance_budgets_1.generateBudgetReport)(budgetResults, budgetReportPath);
            console.log(`✅ Budget report generated at: ${budgetReportPath}`);
            // Report violations
            const errorViolations = budgetResults.violations.filter((v) => v.budget.severity === "error");
            const warningViolations = budgetResults.violations.filter((v) => v.budget.severity === "warning");
 ...
 */
function generateCISummary(results, outputDir) {
    ...
    if (results["app-startup"]) {
        ...
        if (startup.appStartupTime) {
            summary += `| App Startup Time | ${startup.appStartupTime.toFixed(2)} ms |\n`;
        }
        if (startup.firstContentfulPaint) {
            summary += `| First Contentful Paint | ${startup.firstContentfulPaint.toFixed(2)} ms |\n`;
        }
        if (startup.loadTime) {
    ...
    const hasMemoryMetrics = Object.values(results).some((m) => m.jsHeapUsedSize !== undefined);
    if (hasMemoryMetrics) {
        ...
        for (const [scenario, metrics] of Object.entries(results)) {
            if (metrics.jsHeapUsedSize) {
                ...
                const jsHeapTotalMB = metrics.jsHeapTotalSize
                    ...
                    : "N/A";
                summary += `| ${formatScenarioName(scenario)} | ${jsHeapUsedMB} | ${jsHeapTotalMB} |\n`;
            }
        }
        summary += "\n";
    }
    // Add CPU metrics if available
    const hasCpuMetrics = Object.values(results).some((m) => m.cpuUsage !== undefined);
    if (hasCpuMetrics) {
        ...
        for (const [scenario, metrics] of Object.entries(results)) {
            if (metrics.cpuUsage || metrics.frameRate) {
                ...
                const frameRate = metrics.frameRate
                    ...
                    : "N/A";
                summary += `| ${formatScenarioName(scenario)} | ${cpuUsage} | ${frameRate} |\n`;
            }
        }
        summary += "\n";
    }
