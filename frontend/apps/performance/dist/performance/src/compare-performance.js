...
function compareMetrics(baseline, current, significanceThreshold) {
    const results = [];
...
function generateComparisonReport(results, baselineData, currentData, outputPath) {
...
function generateAllMetricsRows(baseline, current) {
    const rows = [];
    // Get all scenario names from both baseline and current
    const allScenarios = new Set([
        ...Object.keys(baseline),
...
function getAllMetricsInObject(obj1, obj2, prefix = '') {
    const metrics = [];
    ...
    for (const key of allKeys) {
        ...
        if (typeof value1 === 'object' && value1 !== null &&
            ...
            const nestedMetrics = getAllMetricsInObject(value1, value2, prefix ? `${prefix}.${key}` : key);
...
async function main() {
    try {
        ...
        const comparisonResults = compareMetrics(baselineData.metrics, currentData.metrics, significanceThreshold);
        // Generate HTML report
        generateComparisonReport(comparisonResults, baselineData, currentData, options.output);
        // Log results
