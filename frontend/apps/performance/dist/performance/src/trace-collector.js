...
exports.saveTraceToFile = saveTraceToFile;
exports.analyzeTrace = analyzeTrace;
exports.generateTraceReport = generateTraceReport;
exports.collectTrace = collectTrace;
exports.saveTraceAnalysis = saveTraceAnalysis;
...
async function stopTracing(page) {
    if (!isTracing || !activeSession) {
        console.log("Tracing is not started or session is invalid, skipping stopTracing");
        return [];
    }
    try {
        const events = [];
        ...
        activeSession.on("Tracing.dataCollected", (data) => {
        ...
        isTracing = false;
        activeSession = null;
        console.log(`Tracing stopped successfully, collected ${events.length} events`);
        return events;
    }
    catch (error) {
 ...
 */
function saveTraceToFile(traceData, outputFile) {
    try {
 ...
 */
function generateTraceReport(metrics, outputFile, title = "Performance Analysis") {
    try {
        const html = `
      <!DOCTYPE html>
      <html>
 ...
 */
async function collectTrace(page, action) {
    try {
 ...
 */
function saveTraceAnalysis(analysis, filePath) {
...
function getTimeStatusClass(time) {
    if (!time)
        return "";
    if (time < 1000)
        return "status-good";
...
function getGCStatusClass(gcTime, totalTime) {
...
function getSlowFramesStatusClass(slowFrames, totalFrames) {
    if (totalFrames === 0)
        return "";
...
function getFrameTimeStatusClass(frameTime) {
    if (frameTime < 16)
        return "status-good";
    if (frameTime < 33)
        return "status-warning";
...
function getLongTasksStatusClass(count) {
    if (count < 3)
        return "status-good";
    if (count < 10)
        return "status-warning";
...
function getTaskTimeStatusClass(time) {
    if (time < 100)
        return "status-good";
    if (time < 300)
        return "status-warning";
