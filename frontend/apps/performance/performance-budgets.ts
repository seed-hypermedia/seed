...
export interface PerformanceBudget {
  ...
  scenario: string
  threshold: number
  operator: 'lt' | 'lte' | 'gt' | 'gte' // less than, less than or equal, greater than, greater than or equal
...
export const defaultBudgets: PerformanceBudget[] = [
  ...
  // JavaScript execution budgets
  {
    ...
    severity: 'warning',
    description:
      'Script execution time should be less than 500ms during heavy operations',
  },
...
export function checkPerformanceBudgets(
  ...
  budgets: PerformanceBudget[] = defaultBudgets,
): BudgetCheckResult {
  const violations: BudgetViolation[] = []
  const passed: BudgetViolation[] = []
  ...
  for (const budget of budgets) {
    ...
    if (budget.metric.includes('.')) {
      ...
      const parentValue = scenarioMetrics[parent as keyof PerformanceMetrics]
      actualValue =
      ...
      // Use indexing with assertion
      actualValue = scenarioMetrics[
        budget.metric as keyof PerformanceMetrics
      ] as number | undefined
    }
    ...
    const percentageDiff =
      ((actualValue - budget.threshold) / budget.threshold) * 100
...
<body>
  ...
  <div class="summary">
    ...
    <div class="summary-box">
      <div class="summary-number warning-number">${
    ...
    <table class="passed-table">
      ...
      <tbody>
        ...
        <tr>
          ...
          <td>${formatOperator(p.budget.operator)} ${formatValue(
            p.budget.threshold,
            p.budget.metric,
          )}</td>
          <td class="${getDiffClass(
            p.percentageDiff,
            p.budget.operator,
          )}">${formatDiff(p.percentageDiff, p.budget.operator)}</td>
        </tr>
        `,
          )
          .join('')}
 ...
 */
function generateViolationCard(
    ...
    <div class="violation-card ${type}">
      <div class="violation-header">
        <div>
          ...
          <div class="violation-scenario">Scenario: ${formatScenarioName(
            budget.scenario,
          )}</div>
        </div>
        <div>
      ...
      <div class="violation-values">
        <div class="value-box">
          ...
          <div class="value-number actual-value">${formatValue(
            actualValue,
            budget.metric,
          )}</div>
        </div>
        <div class="value-box">
          ...
          <div class="value-number expected-value">${formatOperator(
            budget.operator,
          )} ${formatValue(expectedValue, budget.metric)}</div>
        </div>
        <div class="value-box">
          <div class="value-label">Difference</div>
          <div class="value-number diff-value ${getDiffClass(
            percentageDiff,
            budget.operator,
          )}">${formatDiff(percentageDiff, budget.operator)}</div>
        </div>
      </div>
    </div>
  `
 ...
 */
function formatDiff(diff: number, operator: string): string {
  const sign = diff >= 0 ? '+' : ''
 ...
 */
function getDiffClass(diff: number, operator: string): string {
 ...
 */
export function loadBudgetsFromFile(filePath: string): PerformanceBudget[] {
  try {
    const content = fs.readFileSync(filePath, 'utf-8')
    const budgets = JSON.parse(content) as PerformanceBudget[]
    return budgets
  } catch (error) {
