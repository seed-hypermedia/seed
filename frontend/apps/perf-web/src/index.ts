...
import {AppType, BudgetViolation, PerformanceResult} from './types.js'
...
async function main() {
  ...
  Object.entries(mobileMetrics).forEach(([metric, value]) => {
    const budget = appBudgets.mobile[metric as keyof typeof appBudgets.mobile]
    if (budget && value > budget) {
      violations.push({
        metric: metric as keyof typeof mobileMetrics,
        actual: value,
        limit: budget,
        device: 'mobile',
      })
  ...
  Object.entries(desktopMetrics).forEach(([metric, value]) => {
    const budget = appBudgets.desktop[metric as keyof typeof appBudgets.desktop]
    if (budget && value > budget) {
      violations.push({
        metric: metric as keyof typeof desktopMetrics,
        actual: value,
        limit: budget,
        device: 'desktop',
      })
  ...
  if (violations.length > 0) {
    ...
    violations.forEach((v) => {
      console.error(
        `- ${v.device} ${v.metric}: ${v.actual} (limit: ${v.limit})`,
      )
    })
    process.exit(1)
  } else {
