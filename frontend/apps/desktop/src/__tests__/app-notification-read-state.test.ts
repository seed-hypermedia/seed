...
describe('app notification read state', () => {
  ...
  it('marks events as read and prunes when mark-all advances', async () => {
    ...
    expect(afterEventRead.readEvents).toEqual([
      {eventId: 'event-1', eventAtMs: 1100},
    ])
  ...
  it('persists read state across module reload', async () => {
    ...
    expect(stateBeforeReload.readEvents).toEqual([
      {eventId: 'persisted-event', eventAtMs: 1600},
    ])
    ...
    expect(stateAfterReload.markAllReadAtMs).toBe(
      stateBeforeReload.markAllReadAtMs,
    )
