...
function loadStore(): NotificationReadStore {
  const raw = appStore.get(NOTIFICATION_READ_STATE_KEY) as
...
function readEventsMapToList(
  ...
  return Object.entries(readEvents)
    ...
    .sort(
      (a, b) => b.eventAtMs - a.eventAtMs || a.eventId.localeCompare(b.eventId),
    )
}

function readEventsListToMap(
  readEvents: NotificationReadEvent[],
  ...
  for (const evt of readEvents) {
    ...
    next[evt.eventId] =
      current === undefined ? normalizedTime : Math.max(current, normalizedTime)
  }
  return next
}

function pruneReadEvents(
  readEvents: Record<string, number>,
  markAllReadAtMs: number | null,
) {
...
function getOrCreateAccountState(
  accountUid: string,
): AccountNotificationReadState {
...
function updateAccountState(
  accountUid: string,
  updater: (
    current: AccountNotificationReadState,
  ) => AccountNotificationReadState,
) {
...
async function signedNotificationReadStatePost(
  accountUid: string,
  host: string,
  ...
  const response = await fetch(
    `${normalizeHost(host)}/hm/api/notification-read-state`,
    {
      method: 'POST',
      body,
...
function mergeLocalAndRemoteState(
  ...
  const mergedMarkAllReadAtMs = maxNullable(
    local.markAllReadAtMs,
    remote.markAllReadAtMs,
  )
  ...
  for (const evt of remote.readEvents) {
    ...
    mergedReadEventsMap[evt.eventId] =
      current === undefined ? normalized : Math.max(current, normalized)
  }

  const prunedReadEvents = pruneReadEvents(
    mergedReadEventsMap,
    mergedMarkAllReadAtMs,
  )
...
async function runSync(
  ...
  try {
    const remoteState = await signedNotificationReadStatePost(
      accountUid,
      host,
      {
    ...
    const mergedRemote = await signedNotificationReadStatePost(
      accountUid,
      host,
      {
    ...
    updateAccountState(accountUid, () => ({
      ...
      readEvents: pruneReadEvents(
        readEventsListToMap(mergedRemote.readEvents),
        mergedRemote.markAllReadAtMs,
      ),
      dirty: false,
...
function markEventRead(input: {
  ...
  updateAccountState(input.accountUid, (current) => {
    ...
    if (
      current.markAllReadAtMs !== null &&
      eventAtMs <= current.markAllReadAtMs
    ) {
      return current
    }
    const currentEventAtMs = current.readEvents[input.eventId]
    const nextEventAtMs =
      currentEventAtMs === undefined
        ? eventAtMs
        : Math.max(currentEventAtMs, eventAtMs)
    return {
      ...current,
      readEvents: {
        ...current.readEvents,
