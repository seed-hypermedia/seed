...
function loadStore(): NotificationInboxStore {
  const raw = appStore.get(NOTIFICATION_INBOX_STORE_KEY) as
...
async function fetchResolvedEventsPage(input: {
  pageToken?: string
...
async function loadEventsAfterCursor(input: {
...
function mergeAccountNotifications(
  current: NotificationInboxItem[],
  incoming: NotificationInboxItem[],
) {
  ...
  return Array.from(mergedById.values())
    .sort(
      (a, b) =>
        b.event.eventAtMs - a.event.eventAtMs ||
        b.event.feedEventId.localeCompare(a.event.feedEventId),
    )
    .slice(0, NOTIFICATION_MAX_ITEMS_PER_ACCOUNT)
}

function areNotificationListsEqual(
  left: NotificationInboxItem[],
  right: NotificationInboxItem[],
) {
...
function ingestEventsForAccounts(
...
async function runNotificationIngestPoll() {
  ...
  try {
    ...
    if (!loaded.foundCursor) {
      log.warn(
        'Notification inbox cursor was not found in activity feed scan',
        {
          previousCursor,
