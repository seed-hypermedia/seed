          ...
          if (state === 'completed') {
            ...
            if (process.platform === 'darwin') {
              ...
              try {
                ...
                // For macOS
                // - volumePath: /Volumes/SeedDev"}
                // - appName: SeedDev"}
                // - tempPath: /var/folders/j1/nt9my8kx4lz_y5zt64_s1g6r0000gn/T/SeedUpdate"}
                // - IS_PROD_DEV: true"}
              ...
              try {
                ...
                // Remove quotes around scriptPath and use absolute path
                exec(scriptPath, {detached: true, stdio: 'inherit'})
                setTimeout(() => app.quit(), 1000) // Give the script a chance to start
              } catch (error) {
                log.error(`[AUTO-UPDATE] Installation error: ${error}`)
