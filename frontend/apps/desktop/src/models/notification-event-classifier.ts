...
function blockMentionsAccount(block: HMBlockNode, accountUid: string): boolean {
  ...
  if (Array.isArray(annotations)) {
    for (const annotation of annotations) {
      ...
      if (
        mentioned?.uid === accountUid &&
        (!mentioned.path || mentioned.path.length === 0)
      ) {
        return true
      }
    }
  ...
  return (
    block.children?.some((child) => blockMentionsAccount(child, accountUid)) ??
    false
  )
}

function commentMentionsAccount(
  comment: HMComment | null | undefined,
  accountUid: string,
): boolean {
  if (!comment?.content?.length) return false
  return comment.content.some((block) =>
    blockMentionsAccount(block, accountUid),
  )
}
