...
This document provides a comprehensive overview of the implementation details for the cross-window highlighting system
for Hypermedia IDs.
...
- **Main Process** (`app-windows.ts`, `app-api.ts`): Responsible for managing all application windows and broadcasting
...
export type AppWindowEvent =
  // ... other events
  {key: 'hypermediaHoverIn'; hypermediaId: string} | {key: 'hypermediaHoverOut'; hypermediaId: string}
...
```typescript
export function HypermediaHoverManager({highlightColor = DEFAULT_HIGHLIGHT_COLOR}: HypermediaHoverManagerProps) {
...
- **Styles not applying**: Check that the selector matches elements in the DOM (e.g., `data-blockid` or
  `data-resourceid` attributes)
