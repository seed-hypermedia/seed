...
export function ServerSelectScreen({navigation}: Props) {
  ...
  const handleRemoveServer = useCallback((server: ServerInfo) => {
    Alert.alert('Remove Server', `Remove ${server.name} from your saved servers?`, [
  ...
  return (
    <KeyboardAvoidingView style={styles.container} behavior={Platform.OS === 'ios' ? 'padding' : 'height'}>
      ...
      >
        ...
        <Text style={styles.subtitle}>Enter a Hypermedia server URL or select from saved servers</Text>
        ...
        {knownServers.map((server) => (
          <TouchableOpacity
            key={server.url}
            style={[styles.serverItem, currentServer?.url === server.url && styles.serverItemActive]}
          ...
          >
            ...
            {currentServer?.url === server.url && <Text style={styles.activeLabel}>Active</Text>}
          </TouchableOpacity>
        ))}

        <Text style={styles.hint}>Long press to remove a server</Text>
      </ScrollView>
    </KeyboardAvoidingView>
  )
}
