...
describe('hypermedia-metadata semantic field formats', () => {
  ...
  const fieldFormat = (key: string) => {
    const meta = resolveSchema(ONYX_SCHEMAS['metadata']).schema
  ...
  it('the format-typed reference schemas are valid Onyx schemas', () => {
    expect(validate(ONYX_SCHEMAS['schema/meta-schema'], ONYX_SCHEMAS['hm-url'])).toEqual([])
    expect(validate(ONYX_SCHEMAS['schema/meta-schema'], ONYX_SCHEMAS['schema/ipfs'])).toEqual([])
  })
})
...
describe('classifyRef', () => {
  it('a bundled schema published under the onyx account is hm-bundled (no fetch)', () => {
    ...
    expect(classifyRef(`${ONYX}/document`)).toEqual({kind: 'hm-bundled', name: 'document'})
  })
