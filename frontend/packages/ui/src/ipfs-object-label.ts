...
// object resolves further: a bundled schema blob is named by the page that
// defines it (a schema carries no name of its own); an instance that links its
// schema (`schema: {"/": cid}`) is named by that type.
...
export function useIpfsObjectLabel(cid: string): {kind: IpfsObjectKind; label: string; title: string; named: boolean} {
  ...
  if (bundled)
    return {kind: 'schema', label: schemaName(bundled, bundledName!, cid), title: `${title} — schema`, named: true}
  if (valueIsSchema)
    return {kind: 'schema', label: schemaName(value, 'Schema'), title: `${title} — schema`, named: true}
  if (linkedSchemaCid)
    return {
      kind: 'instance',
      label: linkedSchema ? schemaName(linkedSchema, cid, linkedSchemaCid) : cid,
      title: `${title} — a ${
        linkedSchema ? schemaName(linkedSchema, 'typed object', linkedSchemaCid) : 'typed object'
      }`,
      named: !!linkedSchema,
    }
