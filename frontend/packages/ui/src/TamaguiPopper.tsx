...
type ShiftProps = typeof shift extends (options: infer Opts) => void
  ? Opts
  : never
type FlipProps = typeof flip extends (options: infer Opts) => void
  ? Opts
  : never
...
export function Popper(props: ScopedPopperProps<PopperProps>) {
  ...
  const floating = useFloating({
    ...
    middleware: [
      ...
      typeof offsetOptions !== 'undefined'
        ? offsetFn(offsetOptions)
        : (null as any),
      checkFloating,
    ].filter(Boolean),
  })
...
>(function PopperContent(
  props: ScopedPopperProps<PopperContentProps>,
  forwardedRef,
) {
...
export const PopperArrow = PopperArrowFrame.styleable<PopperArrowExtraProps>(
  function PopperArrow(
    ...
    const primaryPlacement = (
      placement ? placement.split('-')[0] : 'top'
    ) as Sides
