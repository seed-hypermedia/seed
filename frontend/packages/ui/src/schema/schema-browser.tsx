...
export function SchemaBrowserPage({
   ...
   * absent, links go to the canonical Hypermedia site. */
  linkSpace?: string
}) {
  ...
  const {byCid} = useSchemaRegistry(bundled || !cid ? [] : [cid])
  const schema = !cid ? undefined : bundled ? HM_SCHEMAS[bundled] : byCid[cid]
