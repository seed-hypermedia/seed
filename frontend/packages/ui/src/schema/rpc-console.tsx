// The schema-driven API console: call the universal-client RPC surface directly,
...
export function RpcCallPanel({method}: {method: RpcMethod}) {
  ...
  const [value, setValue] = useState<unknown>(() => seedValue(method.input, HM_SCHEMAS))
  const [call, setCall] = useState<CallState>({phase: 'idle'})

  const inputWarnings = useMemo(() => validate(method.input, value, '$', {}, HM_SCHEMAS), [method.input, value])
  const outputWarnings = useMemo(
    () => (call.phase === 'done' ? validate(method.output, call.data, '$', {}, HM_SCHEMAS) : []),
    [method.output, call],
  )
    ...
    <div className="flex flex-col gap-4" data-testid={`rpc-call-${method.key}`}>
      <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
        <div className="flex min-w-0 flex-col gap-2">
          ...
          <SchemaDataEditor schema={method.input} value={value} onValue={setValue} registry={HM_SCHEMAS} />
...
export function RpcConsole({initialKey}: {initialKey?: string}) {
