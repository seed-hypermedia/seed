...
import {HM_SCHEMAS, validate} from '../engine'
import {seedValue} from '../data-editor'
import {RpcCallPanel, rpcMethodForSlug, rpcMethods} from '../rpc-console'
...
describe('rpc catalog (derived from the rpc/method union schema)', () => {
  it('exposes every method of the union with key, input, and output', () => {
    ...
    expect(methods.length).toBe(HM_SCHEMAS['rpc/method'].anyOf.length)
  ...
  it('seeds every method an input that already conforms to its input schema', () => {
    for (const method of rpcMethods()) {
      const seeded = seedValue(method.input, HM_SCHEMAS)
      const warnings = validate(method.input, seeded, '$', {}, HM_SCHEMAS)
