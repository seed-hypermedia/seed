...
import {nameToUrl, HM_SCHEMAS} from '../engine'
import {SchemaRegistryProvider} from '../schema-context'
import {metadataSchemaOf} from '../schema-resolve'
;(globalThis as any).IS_REACT_ACT_ENVIRONMENT = true

const characterMeta = metadataSchemaOf(HM_SCHEMAS['example/character-doc'])!
  ...
  return (
    <QueryClientProvider client={new QueryClient({defaultOptions: {queries: {retry: false}}})}>
      <UniversalAppProvider openUrl={() => {}} openRoute={null} universalClient={{request: async () => ({})} as any}>
        <TooltipProvider>
          <ValueEditorProvider>
            <SchemaRegistryProvider schema={characterMeta} registry={{}} value={metadata}>
