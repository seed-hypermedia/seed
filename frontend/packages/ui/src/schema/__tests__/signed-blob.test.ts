...
import {HM_SCHEMAS, nameToUrl, requiredFieldNames, validate} from '../engine'
import {
  isSignedBlobSchema,
  publishSignedBlob,
  signBlob,
...
describe('signed-blob schemas', () => {
  it('recognizes the built-in blob types and a user-defined extension', () => {
    expect(isSignedBlobSchema(HM_SCHEMAS['change'])).toBe(true)
    expect(isSignedBlobSchema(HM_SCHEMAS['capability'])).toBe(true)
    expect(isSignedBlobSchema(VOTE)).toBe(true)
    expect(isSignedBlobSchema(HM_SCHEMAS['example/person'])).toBe(false)
    expect(isSignedBlobSchema(HM_SCHEMAS['example/character-doc'])).toBe(false)
  })
  it('reads the pinned type tag', () => {
    expect(signedBlobTypeTag(VOTE)).toBe('Vote')
    expect(signedBlobTypeTag(HM_SCHEMAS['comment'])).toBe('Comment')
    expect(signedBlobTypeTag(HM_SCHEMAS['blob'])).toBeUndefined()
  })
