...
import {MAP_URL, STRUCT_URL, fieldSchema, nameToUrl, requiredFieldNames} from '../engine'
import {emptyStructSchema, isSignedBlobType, SchemaEditor, withRootKind} from '../schema-editor'
...
describe('schema root kind', () => {
  ...
  const render = (schema: any, onSchema: (s: any) => void = () => {}) =>
    act(() =>
      root.render(
        <AppShell>
          <SchemaEditor schema={schema} onSchema={onSchema} />
        </AppShell>,
      ),
    )
