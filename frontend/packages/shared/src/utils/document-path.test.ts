...
describe('validatePath', () => {
  ...
  // Test reserved paths
  test.each(['/assets', '/favicon.ico', '/robots.txt', '/hm'])('should return error for reserved path: %s', (path) => {
  ...
  test.each(['/-test', '/.test', '/_test'])(
    ...
    (path) => {
      expect(validatePath(path)).toEqual({
        error: `Path can't start with special characters "-", "." or "_": ${path.slice(1)}`,
      })
    },
  )

  // Test valid paths
  test.each(['/valid', '/valid/path', '/valid-path', '/valid_path', '/valid.path', ''])(
    'should return null for valid path: %s',
    (path) => {
