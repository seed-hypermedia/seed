...
>(citations: T[]): T[] {
  ...
  for (const citation of citations) {
    ...
    const targetFragment = citation.targetFragment ? JSON.stringify(citation.targetFragment) : ''
    ...
    if (!latestBySourceAndFragment[key] || time > latestBySourceAndFragment[key].time) {
