...
export function getAllocations(allocation: PaymentAllocation): {
  ...
  if (allocation.mode === 'even') {
    const forEachRecipient = Math.floor((allocation.amount * (1 - SHM_FEE)) / allocation.recipients.length)
    const fee = allocation.amount - forEachRecipient * allocation.recipients.length
  ...
  if (allocation.mode === 'custom') {
    const subTotal = allocation.amounts.reduce((acc, {amount}) => acc + amount, 0)
...
export function applyRecipientAmount(recipientId: string, amountString: string) {
  return (allocation: PaymentAllocation): PaymentAllocation => {
    const amount = Number(amountString)
    if (isNaN(amount)) return allocation
    const customAlloc = allocation.mode === 'custom' ? allocation : evenToCustomAllocation(allocation)
    ...
    const amounts = customAlloc.amounts.map((a) => {
      if (a.account === recipientId) return {account: a.account, amount, ratio: amount / totalAllocFunds}
