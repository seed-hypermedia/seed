...
import {AnyTimestamp, formattedDateDayOnly, formattedDateLong, formattedDateMedium, formattedDateShort} from '../utils'
...
const Translations = {
  Close: 'Cerrar',
  Feed: 'Feed',
  version_count: (args: {count: number}) => `${args.count} ${pluralS(args.count, 'versión', 'versiones')}`,
  comment_count: (args: {count: number}) => `${args.count} ${pluralS(args.count, 'comentario', 'comentarios')}`,
  citation_count: (args: {count: number}) => `${args.count} ${pluralS(args.count, 'cita', 'citas')}`,
  ...
  logout_account_saved: 'Esta cuenta permanecerá accesible en otros dispositivos.',
  ...
  Cancel: 'Cancelar',
  'You will not receive any notifications.': 'No recibirás ninguna notificación.',
  ...
  block_comment_count: (args: {count: number}) => `${args.count} ${args.count === 1 ? 'comentario' : 'comentarios'}`,
  ...
  block_is_collapsed: 'Este bloque está colapsado. Puedes expandirlo y ver sus hijos',
  ...
  link_keys_explainer: (_: any) => {
    return (
      <>
        <p>
          Las cuentas en Hypermedia son basadas en criptografía asimétrica. Tu clave privada está almacenada de forma
          segura, pero solo es accesible desde este navegador en este dispositivo.
        </p>
        <p>
          Para poder volver a iniciar sesión, vincula esta clave a tu cuenta, utilizando las opciones disponibles abajo.
        </p>
      </>
    )
  },
