...
export const ListCommentsByAuthor: HMRequestImplementation<HMListCommentsByAuthorRequest> = {
  async getData(grpcClient: GRPCClient, input): Promise<HMListCommentsByAuthorRequest['output']> {
    ...
    const authors: Record<string, HMMetadataPayload> =
      authorAccountUids.length > 0 ? await loadAccounts(grpcClient, authorAccountUids) : {}
