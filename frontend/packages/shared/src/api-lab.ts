...
export function resolveSchemaNode(
  ...
  return resolveSchemaNode(rootSchema, {...resolvedRef, ...rest}, new Set([...Array.from(visitedRefs), schema.$ref]))
}
...
export function createStarterPayload(
  ...
  if (schema.$ref) {
    ...
    const resolvedRef = resolveLocalRef(rootSchema, schema.$ref)
    const {$ref: _ignoredRef, ...rest} = schema
    return createStarterPayload(
      rootSchema,
      {...resolvedRef, ...rest},
      new Set([...Array.from(visitedRefs), schema.$ref]),
    )
  }
