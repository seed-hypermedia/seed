...
export type HMMetadata = {
  ...
  /** This document DEFINES a schema: ipfs://<cid> of the schema blob it describes. Its target is the meta-schema, so an editor creates and validates the blob as an Onyx schema (the struct form, rooted at Struct by default). Other documents reference this document's URL as their `schema`/`childrenSchema`. */
...
/**
 * Raw citation
 * A citation in raw indexed form, before client-side resolution into a seed-citation. A derived read model computed by the Seed daemon/API for clients — not a sig
...
/**
 * Example Tree
 * Schema: hm://z6MkmZUb4K5c17zGGBuJJerwFzBaGkiYLfEEnkb9CH1W1ptb/sprout-resource
 */
export type SproutResource = {
  height?: number
}
