...
function IndicatorLine({instruction}: {instruction: DropInstruction}) {
  ...
  const style = computeIndicatorStyle(instruction, rect)
...
function computeIndicatorStyle(instruction: DropInstruction, rect: DOMRect): React.CSSProperties | null {
  switch (instruction.type) {
    case 'reorder-above':
      return {
        top: rect.top - INDICATOR_THICKNESS / 2,
        left: rect.left + (instruction.level - 1) * INDENT_PER_LEVEL,
        width: rect.width - (instruction.level - 1) * INDENT_PER_LEVEL,
    ...
    case 'make-child':
      return {
        top: rect.bottom - INDICATOR_THICKNESS / 2,
        left: rect.left + INDENT_PER_LEVEL,
        width: rect.width - INDENT_PER_LEVEL,
        height: INDICATOR_THICKNESS,
      }
    ...
    case 'reparent': {
      ...
      return {
        top: rect.bottom - INDICATOR_THICKNESS / 2,
        left: rect.left - (rect.left - outdent > 0 ? rect.left - outdent : 0),
        width: rect.width + (rect.left - outdent > 0 ? rect.left - outdent : 0),
        height: INDICATOR_THICKNESS,
      }
    }
