...
const writeVerb = {
  ...
  description: [
    ...
    '- `hm://<account>[/<path>]` with a dotted {action} — the raw command surface behind the same verb, for the writes that are not document body content. Extra command fields ride in {input}, never as loose option keys. `capability.grant` delegates access to another account: {action: "capability.grant", input: {delegate: "<account uid>", role: "WRITER" | "AGENT", label}} — the address account is the space granting access and the address path is the scope, so `hm://<account>` grants the whole space and `hm://<account>/docs` grants that subtree (the root scope is the address with no path — an input path of "/" means the same thing and is normalized to it). WRITER lets the delegate write; AGENT hands over full authority over the key, so grant it only when asked for exactly that. `read hm://<account>/:profile` with format "json" lists who already has access. Other raw commands: `profile.update`, `profile.alias`, `contact.create`, `contact.delete`, `comment.update`, `comment.delete`, `draft.create`, `draft.update`, `draft.publish`.',
