  ...
  inputSchema: {
    ...
    properties: {
      ...
      model: {
        type: 'string',
        description:
          'Model child: run the child on one of the models enabled for its agent, as "provider/model" (a bare model id also works when it names exactly one enabled entry). Pick a cheaper/faster model for simple mechanical work and a stronger one for hard reasoning; omit to inherit the agent\'s configured model.',
      },
      tools: {
        type: 'array',
        items: {type: 'string'},
