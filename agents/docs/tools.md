...
| `~/agents`          | the account's agent directory — id, name, model, and enabled models per agent, exactly the set `delegate {agentId}` may target; summaries only, never a sibling's system prompt, grants, or keys. A public agent sees only public siblings — the listing lands in its world-readable transcript |
