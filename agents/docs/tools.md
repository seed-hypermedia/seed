...
| `~/agents`          | the account's agent directory — id, name, model, and enabled models per agent, exactly the set `delegate {agentId}` may target; summaries only, never a sibling's system prompt, grants, or keys                     |
