...
`handoff` on roughly 40% of calls and every one was refused and retried, so `normalizeContinueSessionInput` hoists
those three to the top level when the top level lacks them. `sources` are exact
