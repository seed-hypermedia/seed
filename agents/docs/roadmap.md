...
- QuickJS workflow engine (`run_workflow`): journaled deterministic replay, lint, fuel/memory/journal caps, ctx
...
- `update_plan` live todo snapshots; run actions (GetRun/ListRuns/CancelRun/GetRunJournal) and `runs/<rootRunId>`
  subscriptions;
- Tier-3 live-model validation harness (`agents/e2e/run.ts`).
...
- pinned run/progress card on the session page (in progress on the desktop);
- run the Tier-3 live-model gates (`bun e2e/run.ts all`) — currently blocked on the OpenAI account having no API credits
  — and iterate tool descriptions until the thresholds in workflows-v1-plan.md §15.3 hold;
