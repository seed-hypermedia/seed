...
describe('session store', () => {
  ...
  test('getSession cleans up expired sessions before lookup', () => {
    const db = setupSessionDatabase()
    try {
      const now = Date.now()
      db.run(`INSERT INTO sessions (id, user_id, expire_time, create_time) VALUES (?, ?, ?, ?)`, [
      ...
      expect(store.getSession('active-session')?.user_id).toBe('user-active')
      ...
      expect(ids).toEqual(['active-session'])
    } finally {
      db.close()
    }
  })
